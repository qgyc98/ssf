...
/**
  The function allocates memory for ni sets of cross section parameters that
  will be read/stored via transcrsec procedures.
...
/**
   This method reads one type cross section with different parameters from the opened text file in.
   ...
   Created by Tomas Koudelka koudelka@cml.fsv.cvut.cz, 11. 2001
*/
long csect :: read(XFILE *in)
{
  long i, id;
  char emsg[1001];

  xfscanf(in, "%k%m%k%ld", "crstype", &crsectypet_kwdset, &type, "num_inst", &ninst);
...
*/
long dbcrst::read(XFILE *in)
{
  // reading number of cross section types
  xfscanf(in, "%k%ld", "num_crsec_types", &numt);
 
  crs = new csect[numt]; // allocating memory for each cross section type
  crsu = new long[numt]; 
...
/**
  The method reads cross section characteristics from the opened text file
  with help of transcrsec procedures.

  @param in - pointer to the opened file
  @param transc - pointer to the allocated transcrsec structure
  ...
  Created by TKo, 6.2014
*/
long dbcrst::readtc(XFILE *in, transcrsec *transc, descript *d)
{
  kwd_handling bmode;
  // reading number of cross section types
  xfscanf(in, "%k%ld", "num_crsec_types", &numt);
  ...
  memset(crsu, 0, sizeof(*crsu)*numt);
  transc->ncst = numt;
  transc->cstype  = new crsectypet [numt];  
  transc->numtype = new long [numt];
  for (long i = 0; i < numt; i++)
  {
    xfscanf(in, "%k%m %k%ld", "crstype", &crsectypet_kwdset, &crs[i].type, "num_inst", &crs[i].ninst);
    crs[i].alloctc(crs[i].ninst);
    transc->cstype[i] = crs[i].type;
    transc->numtype[i] = crs[i].ninst;
    if (d->crskwd == no)
    {
      bmode = in->kwdmode;
      if (in->index_created)
    ...
    // reading each index of cross section types
    transc->readcrsectype(in, transc->cstype[i], transc->numtype[i]);
    if (d->crskwd == no)
      in->kwdmode = bmode;
  }
  return 0;
...
/**
   ...
   Created by Tomas Koudelka koudelka@cml.fsv.cvut.cz, 11. 2001
*/
long dbcrst :: search_crs(crsectypet ce, long ci)
{
  long i,ret = -1;
...
*/
void dbcrst::mark_used(long ic, long csti)
{
