...
/*checked 15.11.2006, 6.12.2006 to have the same results as non object-oriented code - smilauer
*/
...
//#include "cmlfile.h" already included in cemhydmat.h
...
//standard constructor when no pointer to existing cemhyd
cemhydmat::cemhydmat(void){
...
- NULL - generate microstructure
- pointer to any cemhyd instance - copies RVE and input parameters

 for next cemhyd instances, inherits the RVE and parameters
*/
cemhydmat::cemhydmat(cemhydmat *CemP)
{
  int i,j,k;
  ...
  average_temp = 0.0;
  n_ipp = 0;
  init_material_time = 0.0;
#endif


  if(CemP!=NULL){
    iseed=CemP->iseed;//initial seed number for random generator
    nseed=iseed;
    seed=(&nseed);
    SYSIZE = CemP->SYSIZE;
    for(k=0;k<SYSIZE;k++){
      for(j=0;j<SYSIZE;j++){
	for(i=0;i<SYSIZE;i++){
	  micorig[i][j][k]=CemP->micorig[i][j][k];
	  mic[i][j][k]=micorig[i][j][k];
	  micpart[i][j][k]=CemP->micpart[i][j][k];
	}
      }
    }
    ...
    ntimes=CemP->ntimes;
    pnucch=CemP->pnucch;
    pscalech=CemP->pscalech;
    pnucgyp=CemP->pnucgyp;
    pscalegyp=CemP->pscalegyp;
    pnuchg=CemP->pnuchg;
    pscalehg=CemP->pscalehg;
    pnucfh3=CemP->pnucfh3;
    pscalefh3=CemP->pscalefh3;
    burnfreq=CemP->burnfreq;
    setfreq=CemP->setfreq;
    phydfreq=CemP->phydfreq;
    outfreq=CemP->outfreq;
    ind_time=CemP->ind_time;
    U_coeff=CemP->U_coeff;
    E_act=CemP->E_act;
    E_act_pozz=CemP->E_act_pozz;
    E_act_slag=CemP->E_act_slag;
    beta=CemP->beta;
    mass_agg=CemP->mass_agg;
    cem_m3=CemP->cem_m3;
    Cp_agg=CemP->Cp_agg;
    Cp_cement=CemP->Cp_cement;
    csh2flag=CemP->csh2flag;
    chflag=CemP->chflag;
    resatcyc=CemP->resatcyc;
    cshgeom=CemP->cshgeom;
    pHactive=CemP->pHactive;
  }
}
...
//initialize common variables from constructor
void cemhydmat::constructor_init(void){
  ...
  LastCallTime = -1.;//start from beginning (the LastCall in the beginning is -1)
  alpha_cur = 0.0;
  alpha_last = 0.;//last degree of hydration
  TargTime_glob = 0.;//stores last call of TargTime 
...
void cemhydmat::matcond (matrix &d,long ri,long ci,long ipp)
{
  long n;
  n = d.n;
...
/**
   ...
   @param ipp - number of integration point
*/
void cemhydmat::matcond1d (matrix &d,long,long,long)
{
  double kk;
...
/**
   ...
   @param ipp - number of integration point
*/
void cemhydmat::matcond2d (matrix &d,long,long,long)
{
  double kk;
...
void cemhydmat::matcond3d (matrix &d,long,long,long)
{
  double kk;
...
/**
   ...
   @param ipp - number of integration point
*/
void cemhydmat::matcap (double &cc,long,long,long)
{
  cc = 0.0;
...
/**
   ...
   @param in - input file
*/
void cemhydmat::read (XFILE *in)
{
  files = new char[1000];

  xfscanf (in,"%lf %lf",&capacity,&conductivity);
  xfscanf(in, "%s", files);//read string - can be %a
  readcem (files);
  delete [] files;
}
...
*/
void cemhydmat::print (FILE *out)
{
  fprintf (out,"  %e %e",capacity,conductivity);
  fprintf (out,"\n%s",files);
}

void cemhydmat::updateval(long ipp,long,long ido){

  double temp, nothing1, nothing2, nothing3, nothing4;
  double actual_time;
  
  if(LastCallTime < 0.0)
    init_material_time = Tp->time;

  actual_time = Tp->time - init_material_time;

  temp = Tm->ip[ipp].other[0];
  temp = temp - 273.15;

  //if cemhydmat has not been already called previously, calculated required values
  if(LastCallTime != actual_time){
    last_values[0] = GetHeat (temp,actual_time);
    last_values[1] = GetDoHActual();//Degree of Hydration
    if(Calculate_elastic_homogenization){
      CreateHDCSH();
      PercolateForOutput();//returns the percolated microstructure in ArrPerc[dx][dy][dz], created from mic_CSH[dx][dy][dz], phases are in phase[]
      AnalyticHomogenizationPaste(last_values[2], last_values[3],0);//cement paste
      AnalyticHomogenizationConcrete(last_values[2], last_values[3], &nothing1, &nothing2, &nothing3, &nothing4, last_values[4], last_values[5]);//concrete
    }
  }

  Tm->ip[ipp].eqother[ido+0] = last_values[0];//released heat - must be in eqother[ido+0] (::get_source)
  Tm->ip[ipp].eqother[ido+1] = last_values[1];//DoH
  Tm->ip[ipp].eqother[ido+2] = last_values[2];//E_paste
  Tm->ip[ipp].eqother[ido+3] = last_values[3];//nu_paste
  Tm->ip[ipp].eqother[ido+4] = last_values[4];//E_concrete
  Tm->ip[ipp].eqother[ido+5] = last_values[5];//nu_concrete
}


void cemhydmat::initvalues(long ipp,long,long ido)
{
  Tm->ip[ipp].eqother[ido+0] = 0.0;//released heat - must be in eqother[ido+0] (::get_source)
  Tm->ip[ipp].eqother[ido+1] = 0.0;//DoH
  Tm->ip[ipp].eqother[ido+2] = 0.0;//E
  Tm->ip[ipp].eqother[ido+3] = 0.5;//nu
  Tm->ip[ipp].eqother[ido+4] = 0.0;//E
  Tm->ip[ipp].eqother[ido+5] = 0.5;//nu  
}

double cemhydmat::get_source(long ipp,long,long ido){

  return (Tm->ip[ipp].eqother[ido+0]);
}
#endif



void cemhydmat::alloc_char_3D(char***(&mic), long SYSIZE){
  mic = new char**[SYSIZE];
...
void cemhydmat::dealloc_char_3D(char***(&mic), long SYSIZE){
  if(**mic!=NULL){
    for (int x=0;x<SYSIZE;x++){
      for (int y=0;y<SYSIZE;y++){
	delete [] mic[x][y];
...
void cemhydmat::alloc_long_3D(long***(&mic), long SYSIZE){
...
void cemhydmat::dealloc_long_3D(long***(&mic), long SYSIZE){
  if(**mic!=NULL){
    for (int x=0;x<SYSIZE;x++){
      for (int y=0;y<SYSIZE;y++){
	delete [] mic[x][y];
...
void cemhydmat::alloc_int_3D(int ***(&mic), long SYSIZE){
...
void cemhydmat::dealloc_int_3D(int***(&mic), long SYSIZE){
  if(**mic!=NULL){
    for (int x=0;x<SYSIZE;x++){
      for (int y=0;y<SYSIZE;y++){
	delete [] mic[x][y];
...
void cemhydmat::alloc_shortint_3D(short int ***(&mic), long SYSIZE){
...
void cemhydmat::dealloc_shortint_3D(short int ***(&mic), long SYSIZE){
  if(**mic!=NULL){
    for (int x=0;x<SYSIZE;x++){
      for (int y=0;y<SYSIZE;y++){
	delete [] mic[x][y];
...
void cemhydmat::alloc_double_3D(double ***(&mic), long SYSIZE){
...
void cemhydmat::dealloc_double_3D(double ***(&mic), long SYSIZE){
  if(**mic!=NULL){
    for (int x=0;x<SYSIZE;x++){
      for (int y=0;y<SYSIZE;y++){
	delete [] mic[x][y];
...
/*read input parameters in file, use cmlfile construction
...
*/
int cemhydmat::readcem (char *inp)
{
  int read_micr;
  ...
  F->set_label_string( 9,"Mass_aggr_frac" ) ;
  F->set_label_string( 10,"Mass_cem" ) ;
  F->set_label_string( 11,"Cp_agg" ) ;
  ...
  if(!read_micr){//generate new microstructure
...
double cemhydmat::ran1(int *idum)
/* Calls: no routines */
...
int cemhydmat::chksph(int xin,int yin,int zin,int radd,int wflg,int phasein,int phase2)
/* Calls: no other routines */
/* Called by: gsphere */
{
...
int cemhydmat::gsphere(int numgen,long int *numeach,int *sizeeach, int *pheach){
...
void cemhydmat::drawfloc(int xin,int yin,int zin,int radd,int phasein,int phase2){
  /* Calls: no other routines */
  /* Called by: makefloc */
  int xp,yp,zp,i,j,k;
  float dist,xdist,ydist,zdist,ftmp;
...
int cemhydmat::chkfloc(int xin,int yin,int zin,int radd){
  /* Calls: no other routines */
  /* Called by: makefloc */
  int nofits,xp,yp,zp,i,j,k;
  float dist,xdist,ydist,zdist,ftmp;
...
/* Called by: runsint */
/* Calls no other routines */
int cemhydmat::maketemp(int size)
{
...
/* Called by rhcalc */
/* Calls no other routines */
int cemhydmat::surfpix(int xin,int yin,int zin){
...
/* Calls surfpix */
/* Called by runsint */
float cemhydmat::rhcalc(int phin){
  int ix,iy,iz;
  long int porc,surfc;
  float rhval;
  ...
  rhval=(float)porc*6.0f/(4.0f*(float)surfc);
...
/* Calls no other routines */
/* Called by sysinit */
int cemhydmat::countem(int xp,int yp,int zp,int phin){
  int xc,yc,zc;
  int cumnum,ic;
...
/* Calls countem */
/* Called by runsint */
void cemhydmat::sysinit(int ph1,int ph2){
  int count,xl,yl,zl;
...
/* Calls no other routines */
/* Called by runsint */
void cemhydmat::sysscan(int ph1,int ph2){
...
/* Calls no other routines */
/* Called by movepix */
int cemhydmat::procsol(int nsearch){
  int valfound,i,stop;
  long int nsofar;
...
int cemhydmat::procair(int nsearch){
  int valfound,i,stop;
  long int nsofar;
...
/* Called by runsint */
int cemhydmat::movepix(int ntomove,int ph1,int ph2){
...
/* Called by main routine */
void cemhydmat::sinter3d(int ph1id,int ph2id,float rhtarget){
...
void cemhydmat::rand3d(int phasein,int phaseout, float xpt){
  int ires;
  ...
  for(i=0;i<31;i++){
	...
	xrad=radius+1-r1;
	filval=s[r1]+(s[r2]-s[r1])*float(xrad);
  ...
  sect=float(resmax-resmin)/500.0f;
  for(i=1;i<=500;i++){
    sum[i]=0.0;
  }
  xtot=0.0;
  ...
  for(i=1;((i<=500)&&(done==0));i++){
    ...
    if(sumtot>xpt){
      ytmp=(float)i;
      vcrit=float(resmin)+float(resmax-resmin)*(ytmp-0.5f)/500.0f;
      done=1;
    }
  }
#ifdef PRINTF
...
void cemhydmat::distrib3d(void)
{
  ...
  */
  ...
  if(volin<1.0){
    rand3d(alumval,alum2, float(volin));
...
void cemhydmat::init(void)
{
  ...
  gypabsprob=0.01f;	/* One sulfate absorbed per 100 CSH units */
  ...
  soluble[C4AF]=1;
  disprob[C4AF]=disbase[C4AF]= float(0.067/DISBIAS);
  creates[C4AF]=POROSITY;
  pHeffect[C4AF]=1.0;
  soluble[C3S]=0;
  disprob[C3S]=disbase[C3S]= float(0.7/DISBIAS);
  creates[C3S]=DIFFCSH;
  pHeffect[C3S]=1.0;
  soluble[C2S]=0;
  disprob[C2S]=disbase[C2S]= float(0.1/DISBIAS);
  creates[C2S]=DIFFCSH;
  pHeffect[C2S]=1.0;
  soluble[C3A]=1;
  /* increased back to 0.4 from 0.25 7/8/99 */
  disprob[C3A]=disbase[C3A]= float(0.4/DISBIAS);
  ...
  disprob[GYPSUM]=disbase[GYPSUM]= 0.025f;   /*geaendert am 04.04.00, urspr. 0.05*/
  ...
  disprob[ANHYDRITE]=disbase[ANHYDRITE]=0.8f*disprob[GYPSUM];
  ...
  disprob[HEMIHYD]=disbase[HEMIHYD]=1.5f*disprob[GYPSUM];   /* ge�ndert am 01.06.00, urspr. 3.0 */
  ...
  soluble[CH]=1;
  disprob[CH]=disbase[CH]= float(0.5/DISBIAS);
  creates[CH]=DIFFCH;
  pHeffect[CH]=0.0;
  /* CaCO3 is only mildly soluble */
  soluble[CACO3]=1;
  disprob[CACO3]=disbase[CACO3]= float(0.10/DISBIAS);
  creates[CACO3]=DIFFCACO3;
  pHeffect[CACO3]=0.0;
  /* Slag is not truly soluble, but use its dissolution probability for reaction probability */
  soluble[SLAG]=0;
  disprob[SLAG]=disbase[SLAG]= float(0.005/DISBIAS);
  creates[SLAG]=0;
  pHeffect[SLAG]=1.0;
  soluble[C3AH6]=1;
  disprob[C3AH6]=disbase[C3AH6]= float(0.01/DISBIAS); /* changed from 0.5 to 0.01 06.09.00 */
  creates[C3AH6]=POROSITY;
  pHeffect[C3AH6]=0.0;
  /* Ettringite is initially insoluble */
  soluble[ETTR]=0;
  /* Changed to 0.008 from 0.020  3/11/99 */
  disprob[ETTR]=disbase[ETTR]= float(0.008/DISBIAS);
  ...
  soluble[CACL2]=1;
  disprob[CACL2]=disbase[CACL2]= float(0.1/DISBIAS);
  creates[CACL2]=DIFFCACL2;
  pHeffect[CACL2]=0.0;
  /* aluminosilicate glass is soluble */
  soluble[ASG]=1;
  disprob[ASG]=disbase[ASG]= float(0.2/DISBIAS);
  creates[ASG]=DIFFAS;
  pHeffect[ASG]=1.0;
  /* calcium aluminodisilicate is soluble */
  soluble[CAS2]=1;
  disprob[CAS2]=disbase[CAS2]= float(0.2/DISBIAS);
  ...
  molarv[POROSITY]=18.068f;
  heatf[POROSITY]=(-285.83f);
  waterc[POROSITY]=1.0f;
  specgrav[POROSITY]=0.99707f;
  ...
  */
  ...
  molarv[CACO3]=36.93f;
  waterc[CACO3]=0.0f;
  specgrav[CACO3]=2.71f;
  heatf[CACO3]=(-1206.92f);
  ...
  molarv[GYPSUM]=74.21f;
  heatf[GYPSUM]=(-2022.6f);
  waterc[GYPSUM]=0.0f;
  specgrav[GYPSUM]=2.32f;
  molarv[GYPSUMS]=74.21f;
  heatf[GYPSUMS]=(-2022.6f);
  waterc[GYPSUMS]=0.0f;
  specgrav[GYPSUMS]=2.32f;

  molarv[ANHYDRITE]=52.16f;
  heatf[ANHYDRITE]=(-1424.6f);
  waterc[ANHYDRITE]=0.0f;
  specgrav[ANHYDRITE]=2.61f;

  molarv[HEMIHYD]=52.973f;
  heatf[HEMIHYD]=(-1574.65f);
  waterc[HEMIHYD]=0.0f;
  specgrav[HEMIHYD]=2.74f;
  ...
  molarv[C3AH6]=150.12f;
  heatf[C3AH6]=(-5548.0f);
  waterc[C3AH6]=6.0f;
  specgrav[C3AH6]=2.52f;
  ...
  molarv[ETTRC4AF]=735.01f;
  heatf[ETTRC4AF]=(-17539.0f);
  waterc[ETTRC4AF]=26.0f;
  specgrav[ETTRC4AF]=1.7076f;
  /* Changed molar volue of ettringite to 735 (spec. gr.=1.7076)  5/24/95 */
  molarv[ETTR]=735.01f;
  heatf[ETTR]=(-17539.0f);
  waterc[ETTR]=26.0f;
  specgrav[ETTR]=1.7076f;
  ...
  heatf[CACL2]=(-795.8f);
  waterc[CACL2]=0.0f;
  specgrav[CACL2]=2.15f;

  molarv[FREIDEL]=296.662f;
  /* No data available for heat of formation */
  heatf[FREIDEL]=(0.0f);
  /* 10 moles of H2O per mole of Freidel's salt */
  waterc[FREIDEL]=10.0f;
  specgrav[FREIDEL]=1.892f;
  ...
  molarv[POZZCSH]=101.81f;
  waterc[POZZCSH]=3.9f;
  specgrav[POZZCSH]=1.884f;
  heatf[POZZCSH]=(-2299.1f);
  ...
  molarv[ABSGYP]=74.21f;
  heatf[ABSGYP]=(-2022.6f);
  waterc[ABSGYP]=0.0f;
  specgrav[ABSGYP]=2.32f;

  molarv[EMPTYP]=18.068f;
  heatf[EMPTYP]=(-285.83f);
  waterc[EMPTYP]=0.0f;
  specgrav[EMPTYP]=0.99707f;
  ...
  specgrav[SLAG]=2.87f;
  specgrav[SLAGCSH]=2.35f;
  molarv[SLAG]=945.72f;
  molarv[SLAGCSH]=1717.53f;
  slagcasi=1.433f;
  slaghydcasi=1.35f;
  siperslag=15.0f;
  waterc[SLAGCSH]= float(5.533*siperslag);
  slagc3a=1.0;
  slagreact=1.0;
  ...
  */
  ...
  if(pHactive){
    ...
    fscanf(slagfile,"%lf",&slagin);
    fscanf(slagfile,"%lf",&slagin);
    specgrav[SLAG]= float(slagin);
    fscanf(slagfile,"%lf",&slagin);
    specgrav[SLAGCSH]= float(slagin);
    fscanf(slagfile,"%lf",&slagin);
    molarv[SLAG]= float(slagin);
    fscanf(slagfile,"%lf",&slagin);
    molarv[SLAGCSH]= float(slagin);
    fscanf(slagfile,"%lf",&slagcasi);
    fscanf(slagfile,"%lf",&slaghydcasi);
    fscanf(slagfile,"%lf",&siperslag);
    fscanf(slagfile,"%lf",&slagin);
    waterc[SLAGCSH]= float(slagin*siperslag);
  ...
  p2slag=(molarv[SLAG]+molarv[CH]* float(CHperslag)+molarv[POROSITY]*(waterc[SLAGCSH]- float(CHperslag)+waterc[C3AH6]* float(slagc3a))-molarv[SLAGCSH]-molarv[C3AH6]* float(slagc3a))/molarv[SLAG];
  p1slag=1.0f-p2slag;
  p3slag=(molarv[SLAGCSH]/molarv[SLAG])-p1slag;
  p4slag= float(CHperslag)*molarv[CH]/molarv[SLAG];
  p5slag= float(slagc3a)*molarv[C3A]/molarv[SLAG];
  if(p5slag>1.0){
    p5slag=1.0;
...
/* Called by passone */
/* Calls no other routines */
int cemhydmat::chckedge(int xck,int yck,int zck){
...
/* Called by dissolve */
/* Calls chckedge */
void cemhydmat::passone(int low,int high,int cycid,int cshexflag){
...
/* Called by dissolve */
/* Calls no other routines */
int cemhydmat::loccsh(int xcur,int ycur,int zcur,int extent){
...
/* Called by makeinert */
/* Calls no other routines */
int cemhydmat::countbox(int boxsize,int qx,int qy,int qz){
...
/* Called by addrand */
/* Calls no other routines */
int cemhydmat::countboxc(int boxsize,int qx,int qy,int qz){
  int nfound,ix,iy,iz,qxlo,qxhi,qylo,qyhi,qzlo,qzhi;
  int hx,hy,hz,boxhalf;
...
void cemhydmat::makeinert(long int ndesire){
...
void cemhydmat::extslagcsh(int xpres,int ypres,int zpres){
...
void cemhydmat::dissolve(int cycle){
  ...
  if(cycle==1){
    ...
    tot_mass= float(cemmass)+(float)count[POROSITY]+specgrav[INERT]*
      ...
      specgrav[POZZ]*(float)count[POZZ]+ float(CH_mass);
    /* water-to-cement ratio */
    if(cemmass!=0.0){
      w_to_c=(float)count[POROSITY]/(float(cemmass)+
				     specgrav[GYPSUM]*(float)count[GYPSUM]+
				     specgrav[ANHYDRITE]*(float)count[ANHYDRITE]+
				     specgrav[HEMIHYD]*(float)count[HEMIHYD]);
    }
    ...
    totfract= float(count[C3S]+count[C2S]);
    totfract+= float(count[C3A]+count[C4AF]);
    totfract+= float(count[GYPSUM]+count[ANHYDRITE]+count[HEMIHYD]);
    fractwithfill=totfract+ float(count[CACO3]+count[SLAG]+count[INERT]);
    fractwithfill+= float(count[POZZ]+count[CAS2]+count[ASG]+count[CACL2]);
    ...
    mass_water= float(1.0-mass_agg)*(float)count[POROSITY]/tot_mass;
    mass_CH= float((1.0-mass_agg)*CH_mass)/tot_mass;
    ...
    if(cemmass!=0.0){
      s_to_c=(float)(count[INERT]*specgrav[INERT]+
		     ...
		     count[POZZ]*specgrav[POZZ])/float(cemmass);
    }
    else{
      s_to_c=0.0;
    }
    ...
    if(cemmass!=0.0){
      heatfill=(float)(count[INERT]+count[SLAG]+count[POZZ]+count[CACL2]+count[ASG]+count[CAS2])/ float(cemmass);  //units pixels^3/g, inverse to the density
    }
    else{
      heatfill=0.0;
    }
  ...
  for(i=0;i<=EMPTYP;i++){
    ...
    if(i==DIFFC3A){
      heatsum+=((float)count[DIFFC3A]-(float)ctest/2.5f)*heatf[C3A]/molarv[C3A];
    }
    /* assume that all C4A which can, does form ettringite */
    if(i==DIFFC4A){
      heatsum+=((float)count[DIFFC4A]-(float)ctest/2.5f)*heatf[C4AF]/molarv[C4AF];
    }
    ...
    if(i==DIFFGYP){
      ...
      heatsum+=(float)ctest*3.30f*heatf[ETTR]/molarv[ETTR];
      molesdh2o+=(float)ctest*3.30f*waterc[ETTR]/molarv[ETTR];
    }
    else if(i==DIFFCH){
    ...
    /* assume that all diffusing anhydrite leads to gypsum formation */
      ...
      molesdh2o+=(float)count[DIFFANH]*2.0f/molarv[GYPSUMS];
    }
    /* assume that all diffusing hemihydrate leads to gypsum formation */
      ...
      molesdh2o+=(float)count[DIFFHEM]*1.5f/molarv[GYPSUMS];
    }
    else if(i==C3S){
      alpha+=(float)(c3sinit-count[C3S]);
      mass_now+=specgrav[C3S]*(float)count[C3S];
      heat4+=0.517f*(float)(c3sinit-count[C3S])*specgrav[C3S];
    }
    else if(i==C2S){
      alpha+=(float)(c2sinit-count[C2S]);
      mass_now+=specgrav[C2S]*(float)count[C2S];
      heat4+=0.262f*(float)(c2sinit-count[C2S])*specgrav[C2S];
    }
    else if(i==C3A){
	...
	frafm/=frtot;
	frhyg/=frtot;
	heat4+=frafm*1.144f*(float)(c3ainit-count[C3A])*specgrav[C3A];
	heat4+=frhyg*0.908f*(float)(c3ainit-count[C3A])*specgrav[C3A];
	heat4+=frettr*1.672f*(float)(c3ainit-count[C3A])*specgrav[C3A];
      }
    }
    else if(i==C4AF){
	...
	frettr/=frtot;
	frhyg/=frtot;
	heat4+=frhyg*0.418f*(float)(c4afinit-count[C4AF])*specgrav[C4AF];
	heat4+=frettr*0.725f*(float)(c4afinit-count[C4AF])*specgrav[C4AF];
      }
    }
    /*               	else if(i==GYPSUM){
    ...
    /* 0.187 kJ/g anhydrite for anhydrite --> gypsum conversion */
      ...
      heat4+=0.187f*(float)(anhinit-count[ANHYDRITE])*specgrav[ANHYDRITE];
      /* 2 moles of water consumed per mole of anhydrite reacted */
      molesh2o+=(float)(anhinit-count[ANHYDRITE])*2.0f/molarv[ANHYDRITE];
    }
    /* 0.132 kJ/g hemihydrate for hemihydrate-->gypsum conversion */
      ...
      heat4+=0.132f*(float)(heminit-count[HEMIHYD])*specgrav[HEMIHYD];
      /* 1.5 moles of water consumed per mole of anhydrite reacted */
      molesh2o+=(float)(heminit-count[HEMIHYD])*1.5f/molarv[HEMIHYD];
    }
  }
  ...
  if(flyashmass!=0.0){
    alpha_fa_cur=1.0f-(mass_fa_now/flyashmass);
  }
  else{
    alpha_fa_cur=0.0f;
  }
  ...
  heat4+=0.80f*((float)nasr/1.3267f)*specgrav[ASG];
  ...
  water_left=(long int)((h2oinit-molesh2o)*molarv[POROSITY]+0.5f);
  ...
  if(cyccnt==0){
    ...
    fprintf(perc_phases,"# Cycle SOL_per SOL_tot| Porosity C3S C2S C3A C4AF GYPSUM HEMIHYD ANHYDRITE POZZ INERT SLAG ASG CAS2 CH CSH C3AH6 ETTR ETTRC4AF AFM FH3 POZZCSH SLAGCSH CACL2 FREIDEL STRAT GYPSUMS CACO3 AFMC AGG ABSGYP EMPTYP HDCSH\n");
  }
  ...
  if((float)count[DIFFCH]>=CHCRIT){
    disprob[CH]=disbase[CH]*float(CHCRIT)/(float)count[DIFFCH];
  }
  else{
...
#endif
  disprob[CH]*= float((A0_CHSOL-A1_CHSOL*temp_cur)/(A0_CHSOL-A1_CHSOL*25.0));
  if((ppozz>0.0f)&&(nfill>0)){
    disprob[CH]*=ppozz/float(PPOZZ);
  }
#ifdef PRINTF
  printf("to %f \n",disprob[CH]);
#endif
  ...
  tdisfact= float(A0_CHSOL-temp_cur*A1_CHSOL);
  ...
  if((ncsbar+anhinit+heminit)==0.0){
    cs_acc=1.0;
    ca_acc=1.0;
    dismin_c3a= float(5.0*DISMIN_C3A_0);
    dismin_c4af= float(5.0*DISMIN_C4AF_0);
  }
  else{
    sulf_conc=sulf_cur*tfractw05*pfractw05/totfract/pfract;
    sulf_conc*=(1000000.0f/float(SYSIZE_POW3));//fixed
    if(sulf_conc<10.0f){
      cs_acc=1.0;
      ca_acc=1.0;
      dismin_c3a= float(DISMIN_C3A_0);
      dismin_c4af= float(DISMIN_C4AF_0);
    }
    else if(sulf_conc<20.0){
      cs_acc=1.0f+((sulf_conc)-10.0f)/10.0f;
      ca_acc=1.0f;
      dismin_c3a= float(DISMIN_C3A_0);
      dismin_c4af= float(DISMIN_C4AF_0);
    }
    else{
      cs_acc= float(1.0+log10(double(sulf_conc)-10.0));
      ca_acc=1.0f;
      dismin_c3a= float((6.0-log10(double(sulf_conc)))*DISMIN_C3A_0);
      dismin_c4af= float((6.0-log10(double(sulf_conc)))*DISMIN_C4AF_0);
      if(dismin_c3a<DISMIN_C3A_0){dismin_c3a= float(DISMIN_C3A_0);}
      if(dismin_c4af<DISMIN_C4AF_0){dismin_c4af= float(DISMIN_C4AF_0);}
    }
  }
  ...
  disprob[C3S]= float(DISMIN)+dfact*disbase[C3S];
  disprob[C2S]= float(DISMIN2)+dfact*disbase[C2S];
  if(disprob[C3S]>(1.0f*disbase[C3S])){disprob[C3S]=(1.0f*disbase[C3S]);}
  if(disprob[C2S]>(1.0f*disbase[C2S])){disprob[C2S]=(1.0f*disbase[C2S]);}
  ...
  disprob[SLAG]= float(slagreact*(DISMINSLAG+double(dfact*disbase[SLAG]))/10.0);
  if(disprob[SLAG]>(float(slagreact)*disbase[SLAG])){disprob[SLAG]= float(slagreact)*disbase[SLAG];}
  if(disprob[C3S]==disbase[C3S]){disprob[SLAG]= float(slagreact)*disbase[SLAG];}
  disprob[ASG]= float(DISMINASG)+dfact*disbase[ASG]/5.0f;
  if(disprob[ASG]>(1.0f*disbase[ASG])){disprob[ASG]=(1.0f*disbase[ASG]);}
  if(disprob[C3S]==disbase[C3S]){disprob[ASG]=disbase[ASG];}
  disprob[CAS2]= float(DISMINCAS2)+dfact*disbase[CAS2]/5.0f;
  if(disprob[CAS2]>(1.0f*disbase[CAS2])){disprob[CAS2]=(1.0f*disbase[CAS2]);}
  if(disprob[C3S]==disbase[C3S]){disprob[CAS2]=disbase[CAS2];}
  /* Adjust CAS2 solubility */
  /* if too many CAS2 already in solution */
  if(count[DIFFCAS2]> float(DCAS2MAX)){
    disprob[CAS2]=0.0f;
  }
#ifdef PRINTF
  ...
  if((ncsbar+heminit+anhinit)>1000*SYSIZE_POW3/1000000.){//fixed
    ...
    if(disprob[C3A]>(1.0f*disbase[C3A])){disprob[C3A]=(1.0f*disbase[C3A]);}
    if(disprob[C4AF]>(1.0f*disbase[C4AF])){disprob[C4AF]=(1.0f*disbase[C4AF]);}

    /* Location to add in dissolution reduction in calcium sulfate phases */
    /* if needed */
    disprob[GYPSUM]=(disbase[GYPSUM]/15.0f)+dfact1*disbase[GYPSUM];
    if(disprob[GYPSUM]>(disbase[GYPSUM])){disprob[GYPSUM]=(disbase[GYPSUM]);}
    disprob[GYPSUMS]=(disbase[GYPSUMS]/15.0f)+dfact1*disbase[GYPSUMS];
    ...
    if(count[DIFFGYP]>DGYPMAX){
      disprob[GYPSUM]=disprob[GYPSUMS]=0.0f;
    }
    disprob[HEMIHYD]=(disbase[HEMIHYD]/15.0f)+dfact1*disbase[HEMIHYD];
    if(disprob[HEMIHYD]>(disbase[HEMIHYD])){disprob[HEMIHYD]=(disbase[HEMIHYD]);}
    disprob[ANHYDRITE]=(disbase[ANHYDRITE]/15.0f)+dfact1*disbase[ANHYDRITE];
    ...
    disprob[C3A]=4.0f*disbase[C3A];
    disprob[C4AF]=4.0f*disbase[C4AF];
  ...
  if((pHactive==1)&&(count[CSH]>((CSHSCALE*surffract*surffract*totfract*totfract/tfractw04/tfractw04)/8.0))){
    ...
    if(pH_cur>12.75){pHfactor=0.667f;}
    if(pH_cur>13.00){pHfactor=0.333f;}
    if(pH_cur>13.25){pHfactor=0.0f;}
    if(pH_cur>13.75){pHfactor=-0.25f;}
    pHfactor+= float(concsulfate);   /* influence of sulfate on reactivity */
  }
  nhgd=0;
  /* Update molar volume ratios for CSH formation */
  pc3scsh=molarvcsh[cyccnt]/molarv[C3S]-1.0f;
  pc2scsh=molarvcsh[cyccnt]/molarv[C2S]-1.0f;
  ...
  for(xloop=0;xloop<SYSIZE;xloop++){
	...
	if(mic[xloop][yloop][zloop]>OFFSET){
	  ...
	  if(((pdis<=(disprob[phid]/(1.+pHfactor*pHeffect[phid])))||((pdis<=(onepixelbias*disprob[phid]/(1.+pHfactor*pHeffect[phid])))&&(micpart[xloop][yloop][zloop]==0)))&&(mic[xc][yc][zc]==POROSITY)){//not fixed yet for one-voxel particles
		...
		if(cshboxsize>=SYSIZE){cshboxsize=SYSIZE;}//11.12.2006 smilauer
	...
	if((count[POZZ]>=13000)&&(chnew<(0.15*SYSIZE*SYSIZE*SYSIZE))&&(csh2flag==1)){
	  if(mic[xloop][yloop][zloop]==CSH){
		...
		plfh3= float(ran1(seed));
		calcx=(19.86f/molarvcsh[cycnew])-(1.0f-calcy);
  ...
  */
  ...
  fchext=1.3f*74*specgrav[C3S]/(specgrav[CH]*228)*(float)discount[C3S]+0.191f*(float)discount[C2S]+
    0.2584f*(float)discount[C4AF];//originally 0.61
  ...
  fc3aext= float(discount[C3A])+0.5917f*(float)discount[C3AH6];
  ...
  fc4aext=0.696f*(float)discount[C4AF];
  nc4aext=(int)fc4aext;
...
/* Called by main program */
/* Calls no other routines */
void cemhydmat::addrand(int randid,long int nneed){
  int ix,iy,iz;
  long int ic;
  int success,cpores;
...
void cemhydmat::measuresurf(void)
{
...
void cemhydmat::resaturate(void)
{
  int sx,sy,sz;
  long int nresat=0;
...
void cemhydmat::outputUnpercImageFile(void){
  FILE *imgout; 
  char extension[10];
  //char outputname[80];
  char *prefix;
  ...
  strcat(prefix, ".img");
#ifdef PRINTF
	printf("name of output file is %s", prefix);
#endif
...
void cemhydmat::readhydrparam()
{
  ...
  printf("%d\n",burnfreq);
  printf("Adiabatic curing conditions %ld\n",adiabatic_curing);
#endif
  //fscanf(in, "%d",&setfreq);
...
void cemhydmat::disrealnew_init(void)
{ 
#ifdef OUTFILES
  char fileo[80];
#endif
 ...
 /* Initialize structure for ants */
  ...
  fprintf(fileperc, "#cycle alpha_burn #through #tot_phase\n");
  disprobfile=fopen("disprob.out","w");
  percfile=fopen("percset.out","w");    //in burnset.cpp	
  fprintf(percfile, "#cycle time alpha_burn #through #C3S+C2S+C3A+C4AF+CAS2+SLAG+ASG+POZZ+ETTR+ETTRC4AF+CSH conn_frac\n");	
  ...
  fprintf(CSHfile, "cyc alpha [h] HD CSH-total\n");
#endif
  ...
  krate= float(exp(-(1000.*E_act/8.314)*((1./(temp_cur+273.15))-(1./298.15))));
  ...
  ppozz= float(PPOZZ*kpozz)/krate;
  /* Assume same holds for dissolution of fly ash phases */
  disprob[ASG]=disbase[ASG]* float(kpozz)/krate;
  disprob[CAS2]=disbase[CAS2]* float(kpozz)/krate;
  /* Update probability of slag dissolution */
  disprob[SLAG]= float(slagreact)*disbase[SLAG]*float(kslag)/krate;
...
/****************************************************************/
  ...
  while(flag==0 ? (time_cur<TargTime) : counter<flag){
    ...
    if(temp_cur<=80.0){
      molarvcsh[icyc]=molarv[CSH]- float(8.0*((temp_cur-20.)/(80.-20.)));
      watercsh[icyc]=waterc[CSH]- float(1.3*((temp_cur-20.)/(80.-20.)));
    }
    else{
      molarvcsh[icyc]=molarv[CSH]-8.0f;
      watercsh[icyc]=waterc[CSH]-1.3f;
    }
    ...
    hydrate(cycflag,ntimes, float(pnucch), float(pscalech), float(pnuchg), float(pscalehg), float(pnucfh3), float(pscalefh3), float(pnucgyp), float(pscalegyp));
    temp_0=temp_cur;
...
#ifdef OUTFILES
    /* Calculate homogenized elastic values */
    if(alpha_cur >= TargDoHelas){
...
|  !LastCallTime  !LastCycHeat   |PrevCycHeat    |             |
|  !              !LastCycCnt    |               |             |
|------------------------------------------------------------------->time
...
double cemhydmat::GetHeat(double GiveTemp, double TargTime){
  //printf("Actual temp %lf, Targ time %lf\n",GiveTemp,TargTime); 
	
  double PartHeat;
  TargTime_glob = TargTime;
  ...
  disrealnew(GiveTemp, TargTime/3600.,0);//perform hydration cycles controlled with TargTime [h]  
  ...
  if(TargTime!=LastCallTime && TargTime!=0.){
    if(LastCycCnt-icyc){//if at least one hydration cycle has elapsed
      //interpolate between CYC_m-1 and CYC_m
      PartHeat=(heat_new*heat_cf-PrevCycHeat)*(TargTime-PrevHydrTime)/(3600.*time_cur-PrevHydrTime);
      ...
      PartHeat=(LastCycHeat-LastTotHeat)*(TargTime-LastCallTime)/(LastHydrTime-LastCallTime);
    }
    
    LastTotHeat+=PartHeat;
    
    if(TargTime != LastCallTime){
      //cem_m3 tells the amount of cement [kg] in a m3 of concrete
...
//TargTime = time_cur*3600.
double cemhydmat::MoveCycles(double GiveTemp, int cycles){
  double PartHeat;
  ...
  PartHeat/=(3600.*time_cur-LastCallTime);
  PartHeat*=1000;
  
  LastCallTime = 3600.*time_cur;
  LastHydrTime = 3600.*time_cur;
  LastCycHeat = (double)(heat_new)*heat_cf;
  LastCycCnt = icyc;

  TargTime_glob = time_cur * 3600.;
...
int cemhydmat::MoveToDoH(double GiveTemp, double DesiredDoH, int maxcyc){
...
double cemhydmat::GetDoHActual(void)
{
  ...
  return alpha_last + (alpha_cur - alpha_last) * (TargTime_glob-PrevHydrTime)/(3600.*time_cur-PrevHydrTime);//interpolate
}


//return cycle in the last CEMHYD cycle (higher or the same as actual time)
int cemhydmat::GetCycNum(void)
{
  return icyc;
}

//return actual time of the last cycle (higher or the same as actual time)
double cemhydmat::GetCycTime(void)
{
  return time_cur;
}
...
int cemhydmat::burn3d(int npix, int d1, int d2, int d3)
...
/* directional flags */
{
  ...
  alpha_burn=1.0f-(mass_burn/float(cemmass));
...
/* Two matrices are used here: one to store the recently burnt locations */
/*                the other to store the newly found burnt locations */
int cemhydmat::burnset(int d1, int d2, int d3)
{
  ...
  for(k=0;k<SYSIZE;k++){
	...
	do{
	  ...
	  for(inew=1;inew<=ncur;inew++){
		...
		/* 2) old pixel is CSH or ETTR or C3AH6 and new pixel is one of cement clinker, slag, of fly ash phases */
...
void cemhydmat::parthyd(void){
  ...
  for(ix=100;ix<=partmax;ix++){
    alpart=0.0;
    if(norig[ix]!=0){
      alpart=1.0f-(float)nleft[ix]/(float)norig[ix];
    }
...
int cemhydmat::moveone(int *xloc, int *yloc,int *zloc,int *act, int sumold)
{
...
int cemhydmat::edgecnt(int xck, int yck, int zck, int ph1, int ph2, int ph3)
{
...
/* Called by hydrate */
int cemhydmat::movecsh(int xcur, int ycur, int zcur, int finalstep, int cycorig)
{
  ...
  /* Changed prcsh limit from 0.1 to 0.01 for CH test 1/27/05 */
    ...
    if(prcsh1<=prtest){
      ...
      cshage[xcur][ycur][zcur]=cyccnt;
      if(cshgeom==1){
	msface=(int)(2.0f*float(ran1(seed))+1.0f);
...
/* Calls moveone and edgecnt */
void cemhydmat::extfh3(int xpres, int ypres, int zpres)
{
...
int cemhydmat::extettr(int xpres, int ypres, int zpres, int etype)
{
...
/* Calls moveone and edgecnt */
void cemhydmat::extgyps(int xpres, int ypres, int zpres)
{
  int multf,numnear,sump,xchr,ychr,zchr,check,fchr,i1,newact;
  long int tries;
...
/* Calls moveone */
/* Called by hydrate */
int cemhydmat::moveanh(int xcur, int ycur, int zcur, int finalstep, float nucprgyp)
{
  ...
  pgen= float(ran1(seed));
  p2diff= float(ran1(seed));
  if((nucprgyp>=pgen)||(finalstep==1)){
    action=0;
    mic[xcur][ycur][zcur]=GYPSUMS;
    count[DIFFANH]-=1;
    count[GYPSUMS]+=1;
    pexp= float(ran1(seed));
    if(pexp<0.4f){
...
/* Called by hydrate */
int cemhydmat::movehem(int xcur, int ycur, int zcur, int finalstep, float nucprgyp)
{
  int xnew,ynew,znew,action,sumback,sumin,check = -1;
  int nexp,iexp,xexp,yexp,zexp,newact,ettrtype;
  float pgen,pexp,pext,p2diff;
...
/* Calls moveone and edgecnt */
int cemhydmat::extfreidel(int xpres, int ypres, int zpres)
{
  int multf,numnear,sump,xchr,ychr,zchr,check,fchr,i1,newact;
  long int tries;
...
/* Calls moveone and edgecnt */
int cemhydmat::extstrat(int xpres, int ypres, int zpres)
{
  int multf,numnear,sump,xchr,ychr,zchr,check,fchr,i1,newact;
  long int tries;
...
/* Called by hydrate */
/* Calls moveone, extettr, extch, and extfh3 */
int cemhydmat::movegyp(int xcur, int ycur, int zcur, int finalstep)
{
...
int cemhydmat::movecacl2(int xcur, int ycur, int zcur, int finalstep)
{
  int check,xnew,ynew,znew,action,nexp,iexp;
...
int cemhydmat::movecas2(int xcur, int ycur, int zcur, int finalstep)
{
  int check,xnew,ynew,znew,action,nexp,iexp;
  int xexp,yexp,zexp,newact,sumold,sumgarb,keep;
  float pexp,pext;
...
int cemhydmat::moveas(int xcur, int ycur, int zcur, int finalstep)
{
  int check,xnew,ynew,znew,action,nexp,iexp;
  int xexp,yexp,zexp,newact,sumold,sumgarb,keep;
  float pexp;
...
int cemhydmat::movecaco3(int xcur,int ycur,int zcur,int finalstep)
{
...
/* Calls moveone and edgecnt */
void cemhydmat::extafm(int xpres, int ypres, int zpres)
{
...
int cemhydmat::moveettr(int xcur, int ycur, int zcur, int finalstep)
{
  int check,xnew,ynew,znew,action;
  int sumold,sumgarb;
...
/* Called by movech */
/* Calls moveone and edgecnt */
void cemhydmat::extpozz(int xpres, int ypres, int zpres)
{
  int check,sump,xchr,ychr,zchr,fchr,i1,action,numnear;
  long int tries;
...
/* Called by hydrate */
/* Calls moveone */
int cemhydmat::movefh3(int xcur, int ycur, int zcur, int finalstep, float nucprob)
{
...
int cemhydmat::movech(int xcur, int ycur, int zcur, int finalstep, float nucprob)
{
...
/* Called by movec3a */
/* Calls moveone and edgecnt */
void cemhydmat::extc3ah6(int xpres, int ypres, int zpres)
{
  int check,sump,xchr,ychr,zchr,fchr,i1,action,numnear;
  long int tries;
...
int cemhydmat::movec3a(int xcur, int ycur, int zcur, int finalstep, float nucprob)
{
  int check,xnew,ynew,znew,action,sumgarb,sumold;
...
/* Called by hydrate */
/* Calls extc3ah6, moveone, extettr, and extafm */
int cemhydmat::movec4a(int xcur, int ycur, int zcur, int finalstep, float nucprob)
{
  int check,xnew,ynew,znew,action,sumgarb,sumold;
  int xexp,yexp,zexp,nexp,iexp,newact;
  float pgen,pexp,pafm,pgrow,p2diff;
...
void cemhydmat::hydrate(int fincyc, int stepmax, float chpar1, float chpar2, float hgpar1, float hgpar2, float fhpar1, float fhpar2, float gypar1, float gypar2)
{
  ...
  for(istep=1;((istep<=stepmax)&&(nleft>0));istep++){
    ...
    beterm= float(exp(-(double)(count[DIFFCH])*1000000./SYSIZE_POW3/chpar2));//fixed
    chprob=chpar1*(1.0f-beterm);
    beterm= float(exp(-(double)(count[DIFFC3A])*1000000./SYSIZE_POW3/hgpar2));//fixed
    c3ah6prob=hgpar1*(1.0f-beterm);
    beterm= float(exp(-(double)(count[DIFFFH3])*1000000./SYSIZE_POW3/fhpar2));//fixed
    fh3prob=fhpar1*(1.0f-beterm);
    beterm= float(exp(-(double)(count[DIFFANH]+count[DIFFHEM])*1000000./SYSIZE_POW3/gypar2));//fixed
    gypprob=gypar1*(1.0f-beterm);
...
void cemhydmat::laguer(fcomplex_cem a[],int m,fcomplex_cem *x,float eps,int polish){
  int j,iter;
...
void cemhydmat::zroots(fcomplex_cem a[],int m,fcomplex_cem roots[],int polish){
  int jj,j,i;
  fcomplex_cem x,b,c;
  fcomplex_cem *ad;
  ...
  for (j=m;j>=1;j--) {
    x=Complex(0.0,0.0);
    laguer(ad,j,&x, float(EPSP),0);
    if (fabs(x.i) <= (2.0*EPSP*fabs(x.r))) x.i=0.0;
    roots[j]=x;
    b=ad[j];
    for (jj=j-1;jj>=0;jj--) {
      c=ad[jj];
  ...
  if (polish)
    for (j=1;j<=m;j++)
      laguer(a,m,&roots[j], float(EPSP),1);
  for (j=2;j<=m;j++) {
    x=roots[j];
    for (i=j-1;i>=1;i--) {
...
void cemhydmat::pHpred(void){//dangerous function - can lead to zero division etc.
  ...
  KspCH= float(KspCH25C*(1.534385-0.02057*temp_cur));
  ...
  pozzreact=((float)npr/1.35f)*float(MASSFACTOR*MASSFACTOR*MASSFACTOR)*specgrav[POZZ];
  ...
  do{  /* while Syngenite precipitating loop */
	...
	coef[0]=Complex(float(C),0.0f);
	coef[1]=Complex(float((A+2.*B*C)/C),0.0f);
	coef[2]=Complex(float(B*B/C+4.0),0.0f);
	coef[3]=Complex(float(4.0*B/C),0.0f);
	coef[4]=Complex(float(4.0/C),0.0f);
	/*     	   printf("coef 0 is (%f,%f)\n",coef[0].r,coef[0].i);
	       printf("coef 1 is (%f,%f)\n",coef[1].r,coef[1].i);
	       printf("coef 2 is (%f,%f)\n",coef[2].r,coef[2].i);
	       printf("coef 3 is (%f,%f)\n",coef[3].r,coef[3].i);
	       printf("coef 4 is (%f,%f)\n",coef[4].r,coef[4].i); */
	roots[1]=Complex(0.0f,0.0f);
	roots[2]=Complex(0.0f,0.0f);
	roots[3]=Complex(0.0f,0.0f);
	roots[4]=Complex(0.0f,0.0f);
	...
	for(j=1;j<=4;j++){
	  ...
	  if(((roots[j].i)==0.0)&&((roots[j].r)>0.0)){
	    ...
	    sumtest= float(concnaplus+conckplus+2.0*double(roots[j].r)-conctest-2.0*concsulfate1);
...
int cemhydmat::IsSolidPhase(int phase){
...
void cemhydmat::burn_phases(int d1, int d2, int d3) {
...
#ifdef OUTFILES
  fprintf(perc_phases,"%d %d ",cyccnt,cnt);
  cnt=0;
#endif
...
int cemhydmat::IsConnected(int cx, int cy, int cz, int dx, int dy, int dz){
...
*/
void cemhydmat::GenerateConnNumbers(void){
  int cx, cy, cz;
  int CentPhase;
...
void cemhydmat::outputImageFilePerc(void){
  FILE *perc_img; 
  char extension[10];
  //char outputname[80];
  char *prefix;
...
void cemhydmat::WriteUnsortedList(int px, int py, int pz){
...
inline int cemhydmat::AdjCoord(int coord){
  if(coord<0){coord+=SYSIZE;}
  if(coord>=SYSIZE){coord-=SYSIZE;}
  return coord;
}


int cemhydmat::NumSol(int cx, int cy, int cz){
  int cnt=0;
  int *p_arr;
...
void cemhydmat::CSHbox(unsigned int *CSH_vicinity){
  int TotPhase=0;
  int cx,cy,cz;
...
void cemhydmat::nrerror(const char *error_text){
...
float* cemhydmat::vector(int nl,int nh){
  float *v;
...
int* cemhydmat::ivector(int nl,int nh){
  int *v;
...
double* cemhydmat::dvector(int nl,int nh){
  double *v;
...
float** cemhydmat::matrix_cem(int nrl,int nrh,int ncl,int nch){

  int i;
  float **m;

  m=(float **) malloc((unsigned) (nrh-nrl+1)*sizeof(float*));
  if (!m) nrerror("allocation failure 1 in matrix_cem()");
  m -= nrl;

  for(i=nrl;i<=nrh;i++) {
    m[i]=(float *) malloc((unsigned) (nch-ncl+1)*sizeof(float));
    if (!m[i]) nrerror("allocation failure 2 in matrix_cem()");
    m[i] -= ncl;
  }
  return m;
}

double** cemhydmat::dmatrix(int nrl,int nrh,int ncl,int nch){
  int i;
  double **m;
...
int** cemhydmat::imatrix(int nrl,int nrh,int ncl,int nch){
  int i,**m;
...
float** cemhydmat::submatrix(float **a,int oldrl,int oldrh,int oldcl,int,int newrl,int newcl){
  int i,j;
  float **m;
...
float** cemhydmat::convert_matrix(float *a,int nrl,int nrh,int ncl,int nch){
  int i,j,nrow,ncol;
  float **m;
...
fcomplex_cem cemhydmat::Cadd(fcomplex_cem a,fcomplex_cem b){	
  fcomplex_cem c;
  c.r=a.r+b.r;
  c.i=a.i+b.i;
  return c;
...
fcomplex_cem cemhydmat::Csub(fcomplex_cem a,fcomplex_cem b){
  fcomplex_cem c;
  c.r=a.r-b.r;
  c.i=a.i-b.i;
  return c;
...
fcomplex_cem cemhydmat::Cmul(fcomplex_cem a,fcomplex_cem b){
  fcomplex_cem c;
  c.r=a.r*b.r-a.i*b.i;
  c.i=a.i*b.r+a.r*b.i;
  return c;
...
fcomplex_cem cemhydmat::Complex(float re,float im){
  fcomplex_cem c;
  c.r=re;
  c.i=im;
  return c;
...
fcomplex_cem cemhydmat::Conjg(fcomplex_cem z){
  fcomplex_cem c;
  c.r=z.r;
  c.i = -z.i;
  return c;
...
fcomplex_cem cemhydmat::Cdiv(fcomplex_cem a,fcomplex_cem b){
  fcomplex_cem c;
  float r,den;
...
float cemhydmat::Cabs(fcomplex_cem z){
  float x,y,ans,temp;
  x=fabs(z.r);
  y=fabs(z.i);
  if (x == 0.0)
...
fcomplex_cem cemhydmat::Csqrt(fcomplex_cem z){
  fcomplex_cem c;
  float x,y,w,r;
  if ((z.r == 0.0) && (z.i == 0.0)) {
    c.r=0.0;
    ...
    if (x >= y) {
      r=y/x;
      w=sqrt(x)*sqrt(0.5f*(1.0f+sqrt(1.0f+r*r)));
    } else {
      r=x/y;
      w=sqrt(y)*sqrt(0.5f*(r+sqrt(1.0f+r*r)));
    }
    if (z.r >= 0.0) {
      c.r=w;
      c.i=z.i/(2.0f*w);
...
fcomplex_cem cemhydmat::RCmul(float x,fcomplex_cem a){
  fcomplex_cem c;
  c.r=x*a.r;
  c.i=x*a.i;
  return c;
...
void cemhydmat::CreateHDCSH(void){
  CSHbox(CSH_vicinity);
}

//public function to call the routine for solid percolation
void cemhydmat::PercolateForOutput(void){
  burn_phases(1,0,0);
}

//public function to access w/c ratio
double cemhydmat::GetWcr(void){
  return w_to_c;
}
...
void cemhydmat::GetInputParams(char *my_string){
  sprintf(my_string, "%d", iseed);
}

long cemhydmat::cx(int x,int y,int z,int a,int b,int c){
  return (1-b-c)*x+(1-a-c)*y+(1-a-b)*z;
}


long cemhydmat::cy(int x,int y,int z,int a,int b,int c){
  return (1-a-b)*x+(1-b-c)*y+(1-a-c)*z;
}

long cemhydmat::cz(int x,int y,int z,int a,int b,int c){
  return (1-a-c)*x+(1-a-b)*y+(1-b-c)*z;
}

//homogenization of plain cement paste without filler
void cemhydmat::AnalyticHomogenizationPaste(double &E, double &nu, int perc_unperc_flag){
  ...
  HomogData CSH_level, Paste_level;
  ...
  double PhaseMatrix[][3]= {{PhaseFrac[1], 0.001,0.499924},//#POROSITY     0
			    {PhaseFrac[2], 135.0,0.3000},  //#C3S          1
			    {PhaseFrac[3], 130.0,0.3000},  //#C2S          2
                            {PhaseFrac[4], 145.0,0.3000},  //#C3A          3 
			    {PhaseFrac[5], 125.0,0.3000},  //#C4AF         4 
			    {PhaseFrac[6], 30.0,0.3000},   //#GYPSUM       5
			    {PhaseFrac[7], 62.9,0.3000},   //#HEMIHYD      6
			    {PhaseFrac[8], 73.6,0.2750},   //#ANHYDRITE    7
			    {PhaseFrac[9], 72.8,0.1670},   //#POZZ,SiO2    8
			    {PhaseFrac[10],79.6,0.3099},   //INERT         9
			    {PhaseFrac[11],72.8,0.1670},   //SLAG         10
			    {PhaseFrac[12],72.8,0.1670},   //ASG          11
			    {PhaseFrac[13],72.8,0.1670},   //CAS2 slag    12 - mechanical prop ?
			    {PhaseFrac[14],38.0,0.3050},   //CH           13
                            {},                            //[15] is from CSH-level 14 // added by TKo, 06.2023 because of wrong dimension of PhaseMatrix in
			    {PhaseFrac[16],22.4, 	0.25},	 //C3AH6	    15
			    {PhaseFrac[17],22.4,	0.25},   //ETTR	            16
			    {PhaseFrac[18],22.4,	0.25},   //ETTRC4AF	    17
			    {PhaseFrac[19],42.3, 	0.3238}, //AFM 	            18
			    {PhaseFrac[20],22.4, 	0.2459}, //FH3 	            19
			    {PhaseFrac[21],22.4, 	0.2459}, //POZZCSH	    20
			    {PhaseFrac[22],22.4, 	0.2459}, //SLAGCSH	    21 - mechanical prop ?
			    {PhaseFrac[23],42.3, 	0.3238}, //CACL2            22
			    {PhaseFrac[24],22.4, 	0.2459}, //FREIDEL	    23
			    {PhaseFrac[25],22.4, 	0.25},	 //STRAT	    24
			    {PhaseFrac[26],30.0, 	0.30},   //GYPSUMS	    25
			    {PhaseFrac[27],0.001,	0.001},	 //CaCO3            26
			    {PhaseFrac[28],42.3, 	0.3238}, //AFMC 	    27
			    {PhaseFrac[29],79.6, 	0.3099}, //INERTAGG	    28
			    {PhaseFrac[30],30.0, 	0.30},   //ABSGYP           29
			    {PhaseFrac[31],0.001,	0.001}}; //EMPTYP           30
  				//HDCSH PhaseFrac[32]            //HDCSH            31
  ...
  Paste_level.SCS(*PhaseMatrix, 31);
...
void cemhydmat::AnalyticHomogenizationConcrete(double E_paste_inp, double nu_paste_inp, double *E_paste, double *nu_paste, double *E_mortar, double *nu_mortar, double &E_concrete, double &nu_concrete){

  HomogData Paste_level, Mortar_level, Concrete_level;
  ...
  double PhaseMatrix[][3]= {{(Vol_cement_clinker_gypsum + Vol_water)/vol_tot_paste, E_paste_inp, nu_paste_inp},//cement paste (reference medium)
			    {Vol_cement_SCM / vol_tot_paste, Young_SCM, Poisson_SCM},//SCM
			    {Vol_inert_filler / vol_tot_paste, Young_inert, Poisson_inert},//inert filler
			 {Vol_entrained_entrapped_air / vol_tot_paste, 0.001, 0.001}};//entrained + entrapped air
  Paste_level.MT_mtrx(*PhaseMatrix, 4, 0);
  ...
  *nu_paste = Paste_level.nu_hmg;
  ...
  double Mortar[][3]= {{Vol_FA / vol_tot_mortar, Young_FA, Poisson_FA},//FA
		       {vol_ITZ_FA/vol_tot_mortar, *E_paste * ITZ_Young_red, *nu_paste},//ITZ
		       {(vol_tot_paste - vol_ITZ_FA)/vol_tot_mortar, *E_paste, *nu_paste},//cement paste
		       {100., 10., 0.3}};//arbitrary values - ref. med.
  Mortar_level.HerveZaoui(*Mortar, 4);
  *E_mortar=Mortar_level.E_hmg;
  *nu_mortar=Mortar_level.nu_hmg;

  //CONCRETE LEVEL
  double vol_tot_concrete = vol_tot_mortar + Vol_CA;
  double Concrete[][3]= {{Vol_CA / vol_tot_concrete, Young_CA, Poisson_CA},//CA
		       {vol_ITZ_CA/vol_tot_concrete, *E_paste * ITZ_Young_red, *nu_paste},//ITZ
		       {(vol_tot_mortar - vol_ITZ_CA)/vol_tot_concrete, *E_mortar, *nu_mortar},//mortar
		       {100., 10., 0.3}};//arbitrary values - ref. med.
  Concrete_level.HerveZaoui(*Concrete, 4);
  E_concrete=Concrete_level.E_hmg;
  nu_concrete=Concrete_level.nu_hmg;
}

//at least one cycle has to pass, volume fraction
void cemhydmat::GetInitClinkerPhases(double &c3s, double &c2s, double &c3a, double &c4af, double &gypsum, double &hemi, double & anh) {
...
BOOST_PYTHON_MODULE(cemhydmat)
{
  boost::python::class_<cemhydmat>("cemhydmat")
    .def("readcem", &cemhydmat::readcem)
    .def("MoveCycles", &cemhydmat::MoveCycles)
    ;
}
#endif
