...
{
  fscanf (in,"%lf %lf %lf  %lf %lf %lf  %lf %lf",&fyc,&fyt,&fybc,&fc,&ft,&fbc,&epsuc,&epsut);
  sra.read (in);
}
...
double chen::yieldfunction (matrix &sig,vector &q)
{
  ...
  if (state==1){
    ...
    if(invar<0.0 && zone<0.0){
      ...
      if (q[0]>epsuc*epsuc)  q[0]=epsuc*epsuc;
      //  yield function with hardening
      f = j2 + ay/3.0*invar - ky + q[0]*((au-ay)/epsuc/epsuc*invar/3.0 - (ku-ky)/epsuc/epsuc);
    }
    //  otherwise
    else{
      ay = (fyc-fyt)/2.0;
      ...
      if (q[0]>epsut*epsut)  q[0]=epsut*epsut;
      //  yield function with hardening
      f = j2 - (invar*invar)/6.0 + ay/3.0*invar - ky +q[0]*((au-ay)/epsut/epsut*invar/3.0 - (ku-ky)/epsut/epsut);
    }
  }
  if (state==2){
    ay = (fybc*fybc-fyc*fyc)/(2.0*fybc-fyc);
...
void chen::deryieldfdsigma (matrix &sig,vector &q,matrix &dfds)
{
  ...
  if (state==1){
    ...
    if(invar<0.0 && zone<0.0){
      ...
      dfds[0][0]=1.0/3.0*(2.0*sig[0][0]-sig[1][1]-sig[2][2]) + ay/3.0 + q[0]/3.0*(au-ay)/epsuc/epsuc;
      dfds[1][1]=1.0/3.0*(2.0*sig[1][1]-sig[0][0]-sig[2][2]) + ay/3.0 + q[0]/3.0*(au-ay)/epsuc/epsuc;
      dfds[2][2]=1.0/3.0*(2.0*sig[2][2]-sig[0][0]-sig[1][1]) + ay/3.0 + q[0]/3.0*(au-ay)/epsuc/epsuc;
    ...
    //  otherwise
      ...
      dfds[0][0]=1.0/3.0*(2.0*sig[0][0]-sig[1][1]-sig[2][2]) + ay/3.0 - 1.0/3.0*invar + q[0]/3.0*(au-ay)/epsut/epsut;
      dfds[1][1]=1.0/3.0*(2.0*sig[1][1]-sig[0][0]-sig[2][2]) + ay/3.0 - 1.0/3.0*invar + q[0]/3.0*(au-ay)/epsut/epsut;
      dfds[2][2]=1.0/3.0*(2.0*sig[2][2]-sig[0][0]-sig[1][1]) + ay/3.0 - 1.0/3.0*invar + q[0]/3.0*(au-ay)/epsut/epsut;
...
void chen::deryieldfdq (matrix &sig,vector &q,vector &dfdq)
{
  ...
  if (state==1){
    ...
    if(invar<0.0 && zone<0.0){
      ...
      dfdq[0] = (au-ay)/epsuc/epsuc*invar/3.0 - (ku-ky)/epsuc/epsuc;
    }
    //  otherwise
    else{
      ay = (fyc-fyt)/2.0;
      ...
      dfdq[0] = (au-ay)/epsut/epsut*invar/3.0 - (ku-ky)/epsut/epsut;
    }
  }
  if (state==2){
    ay = (fybc*fybc-fyc*fyc)/(2.0*fybc-fyc);
...
void chen::deryieldfdsigmadq (matrix &sig,vector &q,matrix &dfdsdq)
{
  ...
  if (state==1){
    ...
    if(invar<0.0 && zone<0.0){
      ...
      dfdsdq[0][0]=(au-ay)/epsuc/epsuc/3.0;
      dfdsdq[1][0]=(au-ay)/epsuc/epsuc/3.0;
      dfdsdq[2][0]=(au-ay)/epsuc/epsuc/3.0;
