...
void creepb::creepinit (long ipp,long ido, double val,nonmechquant nmq)
{
  long nc=Mm->ip[ipp].ncompstr;
  if(nmq == rel_hum )Mm->ip[ipp].eqother[ido+nc*(nRetTime+1)+1]=val;
  if(nmq == temperature )Mm->ip[ipp].eqother[ido+nc*(nRetTime+1)+2]=val;
}
...
void creepb::nlstresses (long ipp, long /*im*/, long ido)
{
  ...
  get_h (ipp, ido);     //initial  moisture
  get_temp (ipp, ido);     //initial  temperature
...
void creepb::phase1 (long ipp,long /*im*/, long ido)
{
  ...
  for (i=0;i<nRetTime;i++){
    ...
    for (j=0; j<nc; j++){
      screep[j][i]=Mm->ip[ipp].eqother[ido+ii+j];	
  ...
  //Mm->stiff_deps_creep (ipp, im, epscr, nc,nc);  // save sig//tady opravit??!!
}



void creepb::phase2 (long ipp, long ido)
{
//  new total strain and stress
  ...
  for (i=0; i<nc; i++){
    depstot[i]=Mm->ip[ipp].strain[i]-Mm->ip[ipp].eqother[ido+i];
  } //increment deps
  qq=2.0/3.0;
  for (i=0;i<nRetTime;i++){
    ii=nc*(i+1);
  ...
  //        fprintf (Out,"\n %le %le %le ",Mm->ip[ipp].eqother[ido+0],Mm->ip[ipp].eqother[ido+1],Mm->ip[ipp].eqother[ido+2]);
  //        fprintf (Out,"\n %le %le %le ",Mm->ip[ipp].eqother[ido+3],Mm->ip[ipp].eqother[ido+4],Mm->ip[ipp].eqother[ido+5]);
  //        fprintf (Out,"\n\n delta eps");
  ...
  for (i=0; i<nc; i++){Mm->ip[ipp].eqother[ido+i]=Mm->ip[ipp].strain[i];}// it is last strain in arr. strain
  //  time history
  seps_time (screep,sig);
  for (i=0; i<nRetTime; i++){
    ii=nc*(i+1);
    for (j=0; j<nc; j++){
      Mm->ip[ipp].eqother[ido+ii+j]=screep[j][i];
    }
  }
  ...
  if (ipp==0){
    fprintf (Out,"\n\n tau faze2");
    //fprintf (Out," %le %le %le %le %le %le",screep[0][0],screep[1][0],screep[2][0],screep[3][0],screep[4][0],screep[5][0]);
    fprintf (Out," %le %le %le %le",screep[0][0],screep[1][0],screep[2][0],screep[3][0]);
  ...
  */
  ...
  Mm->ip[ipp].eqother[ido+nc*(nRetTime+1)+0]+=desht;
  Mm->ip[ipp].eqother[ido+nc*(nRetTime+1)+1]=h_s;
  Mm->ip[ipp].eqother[ido+nc*(nRetTime+1)+2]=temp_s;
  //       fprintf (Out,"\n\n desht hs, temps faze2");
  //       fprintf (Out," %le  %le %le ",desht, h_s, temp_s);

  //  irreversible strain (from t=0)
    for (i=0; i<nc; i++){
      Mm->ip[ipp].eqother[ido+nc*(nRetTime+1)+3+i]+=depstot[i]-deps[i];
      Mm->ip[ipp].eqother[ido+nc*(nRetTime+2)+3+i]+=sig[i];
    }
}

void creepb::get_h (long ipp, long ido)
{
  h_slast=1.0;
  if(type_h !=1 ){h_slast=Mm->ip[ipp].eqother[ido+nc*(nRetTime+1)+1]; }
...
void creepb::get_temp (long ipp, long ido)
{
  temp_slast=283.0;   //10 C
  if(type_temp != 1 ){temp_slast=Mm->ip[ipp].eqother[ido+nc*(nRetTime+1)+2]; }
  
  //if (ipp==0)
  //fprintf (Out,"\n temp_slast %e",temp_slast);
...
/**
  ...
  10.10.2002
*/
void creepb::matstiff (matrix &d, long ipp, long ido)
...
//  d - elastic stiffness matrix
{
  ...
  if(type_h!=1 && type_temp!=1){
    matstiffchange(qq, ipp, ido);
  }
  else{
    matstiffkons(qq);
    //fprintf (Out,"\n ipp %ld   et %le e0 %le",ipp,et,e0);
  }
...
void creepb::matstiffkons (double &qq)
//  function returns elastic stiffness matrix
{
  //  New material modulus in Time
  qq=et/e0;
...
void creepb::matstiffchange (double &qq, long ipp, long ido)
//  function returns elastic stiffness matrix
//  d - elastic stiffness matrix
{
  long i,j,k;
  ...
  if((ccTime+ddTime)==ddTime) {
	...
	if(ss==planestress){
	 for (i=0;i<nRetTime;i++){
		...
		screep[2][i]=exp(-dj)*screep[2][i]+ sig[2]*2.*(1.+mi)*(1.-exp(-dj))/dj/ert[i];
		screep[3][i]=0.0;
	 }
	}
...
*/
void creepb::giveirrstrains (long ipp, long ido, vector &epscr)
{
  long i;
  long ncompstr = Mm->ip[ipp].ncompstr;
  long id=ncompstr+ncompstr*nRetTime+3;
  for (i=0;i<epscr.n;i++)
    epscr[i] = Mm->ip[ipp].eqother[ido+id+i];
}
