...
/**
  ...
  @param[in] out - pointer to the opened output text file
  ...
  Modified by Tomas Koudelka, 10.2008
*/
void ortodam::print (FILE *out)
{
  fprintf (out, "%d ", damevf);
  switch(damevf)
  {
    case brittle:
      fprintf (out,"%le %le ", ft, uft);
      fprintf (out,"%le %le ", fc, ufc);
      fprintf (out,"%d ", cde);
      if (cde != corr_off)
        sra.print (out);      
      break;
    case quasi_brittle:
      ...
      {
        case corr_off:
          fprintf (out, "%le %le %le ", ac, bc, y0c);
          fprintf (out, "%le %le %le ", at, bt, y0t);
          break;
        case corr_on:
          fprintf (out, "%le %le %le ", gfc, bc, y0c);
          fprintf (out, "%le %le %le ", gft, bt, y0t);
          break;
        default:
          print_err(" unknown type of correction of disipated energy is required", __FILE__, __LINE__,__func__);
      }
      break;
    case quadbezier:
      fprintf (out,"%le %le %le ", ft, uft, ult);
      if (fabs(uft/ult-0.5) < qlimit)
        pqt = 1;
      fprintf (out,"%le %le %le ", fc, ufc, ulc);
      if (fabs(ufc/ulc-0.5) < qlimit)
        pqc = 1;
      fprintf (out,"%d ", cde);
      if (cde != corr_off)
...
/**
  ...
  @param[in] nu   - Poissons ratio
  @param[in] i    - principal direction id
  @param[in] peps - %vector of principal strains
...
//        kappa = sqrt(kappa);                 // kappa_I = sqrt(dsig_I*eps_I/dD_I / E)
    ...
    case norrankine:
      ...
      kappa  = (1-nu)*peps(i) + nu*(peps((i+1)%3) + peps((i+2)%3));     //        effective sig(i)/E
      kappa /= (1-2*nu)*(1+nu);
      kappa = kappa;
      break;
    default:
...
/**
  ...
  @param[in]   nu   - Poissons ratio
  @param[in]   peps - %vector of principal strains
  @param[in]   damt - %vector actual values of principal damage parameters for tension
  @param[in]   damc - %vector actual values of principal damage parameters for compression
  @param[in]   aat  - actual values of material parameter a for tension 
  @param[in]   aac  - actual values of material parameter a for compression 
  @param[out]  lft  - %vector of resulting values of load function for tension for each principal direction (output)
  @param[out]  lfc  - %vector of resulting values of load function for compression for each principal direction (output)
...
/**
  ...
  @param[in] ipp - integration point number
  @param[in] y -  parameter of damage function=equivalent strain (generalized conjugated dimensionless thermodynamical force)
  @param[in] e - actual value of Young modulus
  @param[in] f - actual value of strength (tensile or compressive)
  @param[in] uf - actual value of initial crack opening
  @param[in] omegao - old value of damage
...
double ortodam::brittle_damage(long ipp, double y, double e, double f, double uf, double omegao)
{
  ...
  {
    omega = 1.0;
    if (-(kappa-f/e)/(uf-f/e) > min_exp_arg){ // prevent exponent overflow of exp() function
      omega -= (f/(e*kappa))*exp(-(kappa-f/e)/(uf-f/e));
    }
    return omega;
  }
  ...
  omega = 1.0;
  tmp = 0.0;
  if (-h*omega*kappa/uf > min_exp_arg){ // prevent exponent overflow of exp() function
    for (i = 0; i < ni; i++) {
...
/**
  ...
  @param[in] f - actual value of strength (tensile or compressive)
  @param[in] uf - actual value of initial crack opening
  @param[in] ul - actual value of limit crack opening
  @param[in] omegao - old value of damage
  @param[in] pq - indicator of singularity in quadratic Beziere function (pq=1), pure quadratic function have to be used instead of its.
...
/**
  ...
  @param[in]  ipp  - integration point number
  @param[in]  peps - %vector of principal strains
  @param[in]  aat  - actual value of material parameter at 
  @param[in]  aac  - actual value of material parameter ac 
  @param[out] pdamc - %vector of principal damage parameters for tension (output)
  @param[out] pdamt - %vector of principal damage parameters for compression (output)
        ...
        if (kappa >= 0.0)
          {
            ...
            // actual value of limit crack opening
            aul = ult*(af/ft);
            pdamt(i) = qbezier_damage(ipp, fabs(kappa), e, af, auf, aul, pdamt(i),pqt);
          }
        // compression 
        if (kappa < 0.0)
        {
          ...
          // actual value of limit crack opening
          aul = ulc*(af/fc);
          pdamc(i) = qbezier_damage(ipp, fabs(kappa), e, af, auf, aul, pdamc(i),pqc);
        }
      }
      break;
    default:
...
/**
  ...
  @param[out] aat - actual value of parameter At 
  @param[out] aac - actual value of parameter Ac 
...
{
  double aat=0.0, aac=0.0;
  if (damevf == quasi_brittle)
...
/**
  ...
  @param[in] ipp - integration point number
  @param[in] ido - index of internal variables for given material in the ipp other array
  @param[out] d - allocated matrix structure for material stiffness %matrix in the reduced format
  
  @return The function returns stiffness %matrix in the parameter d.

  Created by Tomas Koudelka,
*/
void ortodam::matstiff (long ipp, long ido, matrix &d)
{
...
/**
  ...
  @param[in]  ipp - integration point number
  @param[out] d   - allocated matrix structure for material stiffness %matrix in reduced format

  @return The function returns elastic stiffness %matrix in the parameter d.

  Created by Tomas Koudelka,
*/
void ortodam::elmatstiff (long ipp, matrix &d)
{
  double e, e0;
...
/**
  The function computes secant material stiffnes %matrix.

  @param[in]  ipp - integration point number
  @param[in]  ido - index of internal variables for given material in the ipp other array
  @param[out] d   - allocated matrix structure for material stiffness %matrix in the reduced format according to stress/strain state
...
*/
void ortodam::secmatstiff (long ipp, long ido, matrix &d)
{
  long i;
  
  // actual elastic parameters
  ...
  vector peps(ASTCKVEC(3)), epsn(ASTCKVEC(ncompstr));
  // tensor form of strains and transformation matrix for principal direction of strains
  matrix epst(ASTCKMAT(3,3)), t(ASTCKMAT(3,3));
  // full stiffness matrix (6x6) in the principal coordinate system
  matrix pdf(ASTCKMAT(6,6)), gdf(ASTCKMAT(6,6));
  // normal component blocks of the stiffness and damage matrices
  matrix pdam(ASTCKMAT(3,3)), pdn(ASTCKMAT(3,3)), auxm(ASTCKMAT(3,3));
  // stress/strain state
  strastrestate ssst = Mm->ip[ipp].ssst;


  // total principal strains must be calculated from local values
  for (i=0;i<ncompstr;i++)
    epsn(i) = Mm->ip[ipp].strain[i];

  vector_tensor(epsn, epst, ssst, strain);
  princ_val(epst, peps, t, nijac, limit, Mp->zero, 3, 1);

  // assemble elastic stiffness matrix
  pelmatstiff (ipp, e, nu, pdf);
  for (i=3; i<6; i++)
    pdf(3,3) = pdf(4,4) = pdf(5,5) = g;

  // modify normal stiffness with respect to damage
  identm(pdam);
  for (i=0; i<3; i++){    
    if (peps(i) >= 0.0){ // damage in tension
      pd =  Mm->ip[ipp].other[ido+2+i];
      if (pd > 0.999)
        pd = 0.999;
      /*if (pd < 0.9999)
        pdam(i,i) -= pd;
      ...
      pdam(i,i) -= 0.9999;*/
      pdam(i,i) -= pd;
      pdam(i,i) = sqrt(pdam(i,i));
    }
    else { // damage in compression
      pd = Mm->ip[ipp].other[ido+2+3+i];
      if (pd > 0.999)
        pd = 0.999;
      /*if (pd < 0.9999)
        pdam(i,i) -= pd;
  ...
  extractblock(pdf, pdn, 0, 0);
  mxm(pdam, pdn, auxm);
  mxm(auxm, pdam, pdn);
  storeblock(pdf, pdn, 0, 0);
  
  // transform stiffness matrix to the global coordinate system
  lg_tens4transf(gdf, pdf, t, ssst);
  // assemble stiffness matrix in the reduced format according to stress/strain state
  tensor4_ematrix(d, gdf, ssst);
}
...
/**
  ...
  @param[in]  e   - actual value of elastic modulus
  @param[in]  nu -  actual value of Poisson's ratio
  @param[out] d   - allocated matrix structure for material stiffness %matrix in reduced format according stress/strain state
  ...
  Created by Tomas Koudelka, 02.2020
*/
void ortodam::pelmatstiff (long ipp, double e, double nu, matrix &d)
{
  double b;
...
void ortodam::nlstresses (long ipp, long /*im*/, long ido)
{
  ...
  //double yo;
  vector pyto(ASTCKVEC(3)), pyco(ASTCKVEC(3));
  ...
  matrix sigt(ASTCKMAT(3,3)), d(ASTCKMAT(3,3));
  ...
  //double lf;
  vector peps(ASTCKVEC(3)), psig(ASTCKVEC(3)), lft(ASTCKVEC(3)), lfc(ASTCKVEC(3)), pepsp(ASTCKVEC(3));
  double h;
  long eid;
  ...
  if ((Mp->matmodel == nonlocal) && (Mm->ip[ipp].hmt & 2)){
    ...
    if (Mp->nonlocphase == 1){
      // in case of nonlocal damage, nothing is needed to be computed in the first phase,
