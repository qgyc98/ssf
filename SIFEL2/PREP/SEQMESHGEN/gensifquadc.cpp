...
/*
  Created by Tomas Koudelka, 15.3.2019
*/
{
  long i,j,nx,ny,nn,ne,edg,nid,eid;
  double lx,ly,dx1,dx3,dy1,dy3,xx,yy;
  FILE *s1;
  char *Fname;

  fprintf(stdout, "\n\n\n---          GENERATOR OF RECTANGULAR ELEMENTS ON RECTANGULAR DOMAIN        ---");
  fprintf(stdout, "\n--- generates rectangular elements with twelve nodes on a rectangular domain ---\n");
  if (argc < 7){
  ...
  dx1 = lx/nx;    dy1 = ly/ny;
  dx3 = dx1/3.0;  dy3 = dy1/3.0;
  ne = nx*ny;
  nn = (nx+1)*(ny*3+1)+2*nx*(ny+1);
  ...
  nid=1;
  xx=0.0;  yy=0.0;
  // generation of the first column of nodes
  for (i=0;i<=ny*3;i++){
  ...
  for (i=0;i<nx;i++){
    yy=0.0;
    // generation of 'coarse' columns of nodes
    for (j=0;j<=ny;j++){
      if (j == 0){
        // ordinary nodes on the bottom edge
	fprintf (s1,"\n%8ld  %15.10le %15.10le %15.10le  4  1 0  2 3  3 1  4 1",nid,xx,yy,0.0);
        nid++;
	fprintf (s1,"\n%8ld  %15.10le %15.10le %15.10le  4  1 0  2 3  3 1  4 1",nid,xx+dx3,yy,0.0);
	nid++;
      }
      if (j == ny){
        // ordinary nodes on the top edge
	fprintf (s1,"\n%8ld  %15.10le %15.10le %15.10le  4  1 0  2 1  3 1  4 1",nid,xx,yy,0.0);
        nid++;
	fprintf (s1,"\n%8ld  %15.10le %15.10le %15.10le  4  1 0  2 1  3 1  4 1",nid,xx+dx3,yy,0.0);
        nid++;
      }
      if (j!=0 && j<ny){
        //ordinary inner nodes
	fprintf (s1,"\n%8ld  %15.10le %15.10le %15.10le  3  1 0  3 1  4 1",nid,xx,yy,0.0);
        nid++;
	fprintf (s1,"\n%8ld  %15.10le %15.10le %15.10le  3  1 0  3 1  4 1",nid,xx+dx3,yy,0.0);
        nid++;
      }
      yy+=dy1;
    }
    ...
    // generation of 'dense' column of nodes
    if (i!=nx-1){
      yy=0.0;
      for (j=0;j<=ny*3;j++){
	if (j == 0){
  ...
  // generation of the last node column
  yy=0.0;
  for (i=0;i<=ny*3;i++){
    fprintf (s1,"\n%8ld  %15.10le %15.10le %15.10le",nid,xx,yy,0.0);
    yy+=dy3;
  ...
  for (i=0;i<nx;i++){
    for (j=0;j<ny;j++){
      fprintf (s1,"\n%8ld 6  %8ld %8ld %8ld %8ld %8ld %8ld %8ld %8ld %8ld %8ld %8ld %8ld    1   ",
	       eid,
               // corner nodes
               (i+1)*(ny*3+1)+2*(i+1)*(ny+1)+(j+1)*3+1,
	       i*(ny*3+1)+2*i*(ny+1)+(j+1)*3+1,
	       i*(ny*3+1)+2*i*(ny+1)+j*3+1,
	       (i+1)*(ny*3+1)+2*(i+1)*(ny+1)+j*3+1,
               // mid-side nodes on edge 1
	       (i+1)*(ny*3+1)+2*i*(ny+1)+2*(j+1)+1+1,
	       (i+1)*(ny*3+1)+2*i*(ny+1)+2*(j+1)+1,
               // mid-side nodes on edge 2
	       i*(ny*3+1)+2*i*(ny+1)+j*3+2+1,
	       i*(ny*3+1)+2*i*(ny+1)+j*3+1+1,
               // mid-side nodes on edge 3
	       (i+1)*(ny*3+1)+2*i*(ny+1)+j*2+1,
	       (i+1)*(ny*3+1)+2*i*(ny+1)+j*2+1+1,
               // mid-side nodes on edge 4
	       (i+1)*(ny*3+1)+2*(i+1)*(ny+1)+j*3+1+1,
	       (i+1)*(ny*3+1)+2*(i+1)*(ny+1)+j*3+2+1);
      eid++;
      if (edg)
      {
