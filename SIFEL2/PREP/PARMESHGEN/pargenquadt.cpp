...
int printprep(char *fname, char *afname, char *tmplfn, FILE *in)
{
  ...
  char *ptr;
  FILE *out, *tmplf;
  long i, j, id, did, k, nlc, end;
  char str[1205];
  double *bc1=NULL, *bc3=NULL;
...
//      sprintf(outfn+strlen(outfn), "%ld.out", did+1);
      ...
      if (bc1 == NULL)
      {
        bc1 = new double[nlc];
        bc3 = new double[nlc];
        memset(bc1, 0, sizeof(*bc1)*nlc);
        memset(bc3, 0, sizeof(*bc3)*nlc);
        for (k = 0; k < nlc; k++)
          fscanf(in, "%le %le", &bc1[k], &bc3[k]);
      }
      ...
      if (i == 0)
      {
        fprintf(out, "0 bocon %ld", nlc);
        for (k = 0; k < nlc; k++)
          fprintf(out, " %ld 0 %e", k+1, bc1[k]);
        fprintf(out, "\n");
      }
      // add nodal forces to domains on the right side
      if (i == Ndx-1)
      {
        fprintf(out, "2 bocon %ld", nlc);
        for (k = 0; k < nlc; k++)
          fprintf(out, " %ld 0 %e", k+1, bc3[k]);
        fprintf(out, "\n");
      }
      fprintf(out, "end\n");
