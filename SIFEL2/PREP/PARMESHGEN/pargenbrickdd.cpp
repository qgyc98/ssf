...
void gen_nodes(void)
{
  long dix, djy, dkz, i, j, k, *lid, ni, ngi, ngin, did;
  double xx, yy, zz, lx, ly, lz;
  long bnex, bney, bnez; // backup of number of elements on given domain and given direction
        ...
        // loop over elements in the y direction
        {
          ...
          // loop over domains in the z direction
          {
            ...
            // loop over elements in the z direction
            {
              ...
              Lnn[did][lid[did]] = ni;
              lid[did]++;
              // negative global node numbering
              if ((i == 0 || i == bnex) && (j == 0 || j == bney) && (k == 0 || k == bnez))
              {
                if (Gnn[ni] == 0)
                {
		  Gnn[ni] = ngin;
      ...
      for (k = 0; k < Ndz; k++)
      {
        ...
        for (ii = 0; ii < Nex[i]+1; ii++)
        {
          for (jj = 0; jj < Ney[j]+1; jj++)
          {
            ...
            {
              fprintf(out, "%le %le %le", X[Lnn[did][id]], Y[Lnn[did][id]], Z[Lnn[did][id]]);
              if (ii == 0)
              {
                fprintf(out, " 1\n");
                id++;
...
//        sprintf(outfn+strlen(outfn), "%ld.out", did+1);
        ...
        {
          for (l = 0; l < nlc; l++)
            fprintf(out, "3 nload %ld %le %le %le\n", l+1, fx[l], fy[l], fz[l]);
        }
        fprintf(out, "end\n");
