...
lplate::lplate (int np,int mr,int nd,gtopology *top)
{
  ...
  if (myrank==0){
    ...
    constrmat = new double* [(nl-1)*nn*4];
...
void lplate::assemble_constr_dm ()
{
  ...
  for (i=1;i<nl;i++){
    for (j=0;j<nn;j++){
      ...
      for (k=0;k<ndofn;k++){
	cnd[k]=cn[i-1][j][k]+(i-1)*ndof;
      }
      for (k=0;k<ndofn;k++){
	if (cn[i][j][k]!=0)
	  cnd[k+ndofn]=cn[i][j][k]+i*ndof;
	else
	  cnd[k+ndofn]=cn[i][j][k];
      }
      for (k=0;k<nnmult;k++){
