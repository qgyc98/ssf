...
/**
   ...
   JK
*/
lplate::lplate (int np,int mr,long nd,gtopology *top)
{
  long i,j,k;
...
void lplate::orthonormalization ()
{
  ...
  for (i=1;i<nl-1;i++){
    ...
    for (j=0;j<nn*nnmult;j++){
      alpha=constrmat[ii][i*2+0]*constrmat[jj][i*2+0] + constrmat[ii][i*2+1]*constrmat[jj][i*2+1];
      nor=0.0;
      for (k=0;k<2*(i+2);k++){
...
void lplate::precondstep (gmatrix *gm,double *dd,double *pp)
{
/*
  long i;
...
/**
   ...
   JK, 12.8.2003
*/
void lplate::pcg (gmatrix *gm,double *w,double *rhs,long *domproc,FILE *out)
{
  long i,j;
  double nom,denom,alpha,beta,norrhs,pr1;
  double *d,*dd,*r,*p,*pp,*h;
  MPI_Status stat;
  ...
  if (myrank==0){
    ...
    for (j=1;j<nproc;j++){
      nullv (dd,maxndof+1);
      globloc (r,dd,domproc[j]);
      MPI_Send (dd,maxndof,MPI_DOUBLE,j,ndom,MPI_COMM_WORLD);
    }
    nullv (dd,maxndof+1);
    globloc (r,dd,domproc[0]);
    
    precondstep (gm,dd,pp);
    
    nullv (h,nmult);
    locglob (h,pp,domproc[0]);
    for (j=1;j<nproc;j++){
      MPI_Recv(pp,maxndof,MPI_DOUBLE,MPI_ANY_SOURCE,MPI_ANY_TAG,MPI_COMM_WORLD,&stat);
      locglob (h,pp,domproc[stat.MPI_TAG]);
    }
    ...
    MPI_Recv (dd,maxndof,MPI_DOUBLE,MPI_ANY_SOURCE,MPI_ANY_TAG,MPI_COMM_WORLD,&stat);
    precondstep (gm,dd,pp);
    MPI_Send (pp,maxndof,MPI_DOUBLE,0,ndom,MPI_COMM_WORLD);
			   ...
			   long *domproc,double *lhs,double *rhs,FILE *out)
{
  ...
  */
  ...
  if (myrank==0){
    ...
    MPI_Send(buff,4,MPI_LONG,0,myrank,MPI_COMM_WORLD);
  }
  MPI_Barrier (MPI_COMM_WORLD);
  delete [] buff;
