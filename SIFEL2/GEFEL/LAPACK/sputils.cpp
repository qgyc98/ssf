...
//static void ScaleComplexMatrix(MatrixPtr  Matrix, double *RHS_ScaleFactors, double *SolutionScaleFactors );
//static void ComplexMatrixMultiply(MatrixPtr  Matrix, double *RHS, double *Solution);
//static void ComplexTransposedMatrixMultiply(MatrixPtr  Matrix, double *RHS, double *Solution);
...

#if spCOMPLEX && MULTIPLICATION && TRANSPOSE
/*
...
static void ComplexTransposedMatrixMultiply(MatrixPtr  Matrix, double *RHS, double *Solution IMAG_VECTORS )
{
 ElementPtr  pElement;
...
#endif /* spCOMPLEX && MULTIPLICATION && TRANSPOSE */
...
#if spCOMPLEX
	void spDeterminant(MatrixPtr  Matrix, int *pExponent, double *pDeterminant, double *piDeterminant )
#else
...
/* Scale Determinant. */
            if (*pDeterminant != 0.0)
            {   while (ABSS(*pDeterminant) >= 1.0e12)
                {   *pDeterminant *= 1.0e-12;
                    *pExponent += 12;
                }
                while (ABSS(*pDeterminant) < 1.0e-12)
                {   *pDeterminant *= 1.0e12;
                    *pExponent -= 12;
                }
            }
...
/* Scale Determinant again, this time to be between 1.0 <= x < 10.0. */
        if (*pDeterminant != 0.0)
        {   while (ABSS(*pDeterminant) >= 10.0)
            {   *pDeterminant *= 0.1;
                (*pExponent)++;
            }
            while (ABSS(*pDeterminant) < 1.0)
            {   *pDeterminant *= 10.0;
                (*pExponent)--;
            }
        }
...
#if TRANSLATE && DELETE
/*
 *  DELETE A ROW && COLUMN FROM THE MATRIX
 *
...
void spDeleteRowAndCol(MatrixPtr Matrix, int Row, int Col )
{
	ElementPtr  pElement, *ppElement, pLastElement, spcFindElementInCol();
...
/* Correct Diag pointers. */
    ...
    {
		Matrix->Diag[Row] = spcFindElementInCol(Matrix, Matrix->FirstInCol+Row, Row, Row, 0 );
        Matrix->Diag[Col] = spcFindElementInCol(Matrix, Matrix->FirstInCol+Col, Col, Col, 0);
    }
...
/* Forward elimination. Solves Lw = e while choosing e. */
    ...
    for (I = 1; I <= Size; I++)
    {
        ...
        if (ABSS(Wm) > SLACK)
        {
			ScaleFactor = 1.0 / MAX( SQR( SLACK ), ABSS(Wm) );
...
/* Compute 1-norm of T, which now contains w, and scale ||T|| to 1/SLACK. */
    for (ASw = 0.0, I = Size; I > 0; I--) ASw += ABSS(T[I]);
...
/* Backward Substitution. Solves Uy = w.*/
    ...
    {   pElement = Matrix->Diag[I]->NextInRow;
        ...
        if (ABSS(T[I]) > SLACK)
        {
			ScaleFactor = 1.0 / MAX( SQR( SLACK ), ABSS(T[I]) );
            for (K = Size; K > 0; K--) T[K] *= ScaleFactor;
            E *= ScaleFactor;
        }
    }
...
/* Compute 1-norm of T, which now contains y, and scale ||T|| to 1/SLACK. */
    for (ASy = 0.0, I = Size; I > 0; I--) ASy += ABSS(T[I]);
...
/* Compute infinity-norm of T for O'Leary's estimate. */
    for (MaxY = 0.0, I = Size; I > 0; I--)
        if (MaxY < ABSS(T[I])) MaxY = ABSS(T[I]);
...
/* Compute 1-norm of T, which now contains v, and scale ||T|| to 1/SLACK. */
    for (ASv = 0.0, I = Size; I > 0; I--) ASv += ABSS(T[I]);
...
/* Compute 1-norm of T, which now contains z. */
    for (ASz = 0.0, I = Size; I > 0; I--) ASz += ABSS(T[I]);
...
static double ComplexCondition(MatrixPtr Matrix, double NormOfMatrix, int *pError)
{
	ElementPtr pElement;
	ComplexVector T, Tm;
	int I, K, Row, Size;
	ElementPtr pPivot;
	double E, Em, ASp, ASm, ASw, ASy, ASv, ASz, MaxY, ScaleFactor, Linpack, OLeary, InvNormOfInverse;
	ComplexNumber Wp, Wm;
