...
static void SolveComplexMatrix(MatrixPtr Matrix, double *RHS, double *Solution IMAG_VECTORS)
{
	ElementPtr  pElement;
...
static void SolveComplexTransposedMatrix(MatrixPtr Matrix, double *RHS, double *Solution IMAG_VECTORS)
{
	ElementPtr  pElement;
	ComplexVector  Intermediate;
	int  I, *pExtOrder, Size;
...
/* Cmplx expr: Intermediate[Element->Col] -= Temp * *Element. */
                CMPLX_MULT_SUBT_ASSIGN( Intermediate[pElement->Col], Temp, *pElement);
                pElement = pElement->NextInRow;
            }
        }
    }
