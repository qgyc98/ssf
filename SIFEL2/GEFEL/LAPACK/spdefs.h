/*
 * DATA STRUCTURE && MACRO DEFINITIONS for Sparse.
 *
...
#define REAL YES
#define spCOMPLEX NO /* YES */
#define EXPANDABLE  NO /* YES */
#define TRANSLATE NO /* YES */
#define INITIALIZE YES
#define DELETE YES
#define STRIP YES
#define MODIFIED_NODAL YES
#define QUAD_ELEMENT YES
#define TRANSPOSE NO /* YES */
#define SCALING YES
...
#if spCOMPLEX
#define ELEMENT_MAG(ptr) (ABSS((ptr)->Real) + ABSS((ptr)->Imag))
#else
...
#define CMPLX_MULT(to,from_a,from_b) { (to).Real = (from_a).Real * (from_b).Real - \
...
#define CMPLX_MULT_ASSIGN(to,from) { double to_real_ = (to).Real; \
 ...
 * conjugated. */
#define CMPLX_CONJ_MULT(to,from_a,from_b) { (to).Real = (from_a).Real * (from_b).Real + \
 (from_a).Imag * (from_b).Imag; (to).Imag = (from_a).Real * (from_b).Imag - (from_a).Imag * (from_b).Real; }
...
#define CMPLX_MULT_ADD_ASSIGN(to,from_a,from_b) { (to).Real += (from_a).Real * (from_b).Real - \
 (from_a).Imag * (from_b).Imag; (to).Imag += (from_a).Real * (from_b).Imag + (from_a).Imag * (from_b).Real; }
...
#if DEBUG
#define ABORT() { (void)fflush(stdout); \
...
#define CALLOC(ptr,type,number) { int i; if ((ptr = ALLOC(type, number)) != (type *)NULL) \
 ...
 *
typedef struct { double Real, Imag; } ComplexNumber, *ComplexVector;*/
 ...
 */
struct AllocationRecord { char *AllocatedPtr; struct AllocationRecord *NextRecord; };
 ...
 */
struct FillinListNodeStruct { ElementPtr pFillinList;
...
struct MatrixFrame
{
	...
	BOOLEAN Factored;
	int Fillins;
	ArrayOfElementPtrs FirstInCol, FirstInRow;
	unsigned long ID;
	double* Intermediate;
	...
	struct FillinListNodeStruct *FirstFillinListNode, *LastFillinListNode;
};
