...
BOOL DSSolver::SetMatrixPattern(SparseMatrixF* smt,unsigned char block_size)
{
  ...
  sprintf(str," number of equations : %ld ",m_sm.neq);
  eMT->Writeln(str);
  sprintf(str," number of nonzeros  : %ld nonzeros",m_sm.Nonzeros());
  eMT->Writeln(str);

  if (SolverType==eDSSDiagonalScaling)
    {
      matrixD.Init(neq);
      m_sm.ReadDiagonal(matrixD.DataPtr());
    }
  m_eState = ISolver::Initialized;
  return 1;
}

BOOL DSSolver::IsInitialized()
{
  return m_eState != ISolver::None;
}
...
BOOL DSSolver::IsAllocated()
{
  return m_eState == ISolver::Allocated 
    || m_eState == ISolver::Factorized;
}

BOOL DSSolver::IsSchur()
{
  return m_bIsSchur;
}
...
{
  long no_nonzeros = m_sm.Nonzeros();
  eMT->CS();

  SparseConectivityMtxII* block_conmtx = new SparseConectivityMtxII(m_sm,this->mcn,blockSize);
  ...
  switch(SolverType)
    {
    case eDSSFactorizationLDLTIncomplete:
      matrix = new SparseGridMtxLDL(m_sm,blockSize,order,mcn,eMT,TRUE);
      break;
    case eDSSFactorizationLLTIncomplete:
      matrix = new SparseGridMtxLL(m_sm,blockSize,order,mcn,eMT,TRUE);
      break;
    case eDSSFactorizationLDLT:
      matrix = new SparseGridMtxLDL(m_sm,blockSize,order,mcn,eMT,TRUE);
      break;
    case eDSSFactorizationLLT:
      ...
      break;
    case eDSSFactorizationLU:
      matrix = new SparseGridMtxLU(m_sm,blockSize,order,mcn,eMT,TRUE);
      break;
    case eDSSFastCG:
      matrix = new SparseGridMtxLDL(m_sm,blockSize,order,mcn,eMT,TRUE);
      break;
...
BOOL DSSolver::PreFactorize()
{
  ...
  // just for case delete the original matrix
  delete matrix;
  matrix = NULL;
  delete matrixPD;
  matrixPD = NULL;
    ...
    case eDSSparseMatrix:
      {
        if (IsSchur())
          {	// Schur complement factorization
            return PreFactorizeSchur();
          }
        else
          {	// standard full factorization
...
BOOL DSSolver::LoadNumbers (SparseMatrixF* sm) 
{
  ...
  if (!m_sm.IsSamePattern(sm))
    {
      eMT->Writeln("Matrix has different nonzero pattern!");
      return 0;
    }
  if (m_sm.a==NULL || m_sm.a!=sm->a)
    {
      m_sm = *sm;
    }
...
BOOL DSSolver::AddNumbers (double alfa,SparseMatrixF* smtx)
{
  ...
  m_sm.AddNumbers(alfa,smtx->a);
  return TRUE;
}

BOOL DSSolver::ScaleMatrix(double alfa)
{
  if (m_eState != ISolver::Allocated)
    {
      eMT->Writeln("First load matrix.");
  ...
  m_sm.ScaleMatrix(alfa);
  return TRUE;
}

SparseMatrixF* DSSolver::GetSparseMatrix()
{
  if (m_eState != ISolver::Allocated)
    {
      eMT->Writeln("First load matrix.");
...
BOOL DSSolver::ReFactorize( )
{
  ...
  if (!IsAllocated()){
    eMT->Writeln("The matrix has to be allocated (PreFactorize) prior to the factorization.");
    return FALSE;
  }
  eMT->CS();
...
BOOL DSSolver::LoadMCN(ULONG n_blocks,unsigned char block_size,long * mcn,BOOL bIsSchur)
{
...
BOOL DSSolver::CreateFixedArray(long no_noncondensed_DOFs)
{
  ...
  //	if (!IsSchur() && fixed->Count>0)
  //	{
  //		eMT->Writeln("Warrning: There were fixed nodes, we have to switch IsSchur to TRUE!!");
  //		m_bIsSchur = TRUE;
  //	}
...
BOOL DSSolver::PreFactorizeSchur()
{
  if (mcn == 0)
    {
      eMT->Writeln("The MCN vector was not set! It is needed for Schur complement condensation.");
      return FALSE;
    }
...
void DSSolver::StoreFixedLastPermutation_dom_order()
{
  ...
  dom_order = new IntArrayList(m_sm.neq);
  dom_order->Alloc();
...
void DSSolver::condense(double *a,double *lhs,double *rhs,long tc)
{
  if (!IsSchur())
    {
      if (m_eState == ISolver::Initialized)
        {
          m_bIsSchur = TRUE;// still ok to switch on when in Initialized state
        }
      else
        {
          eMT->Writeln("Error : If you want to call condense you should switch IsSchur to TRUE in LoadMCN!");
          return;
        }
    }
  ...
  if (tc == 1)
    {
      if (m_eState!=ISolver::Factorized)
        if (!Factorize())
          return;
...
{
  m_sm.GetA12block(pA12,lncn->Count);
}
...
void DSSolver::MulMatrixByVector(double *b, double *c)
{
  ...
  if (SolverType==eDSSFactorizationLU)
    m_sm.MulNonsymMatrixByVector(b,c);
  else
    m_sm.MulMatrixByVector(b,c);
  //m_sm.mxv_scr(b,c);
}
