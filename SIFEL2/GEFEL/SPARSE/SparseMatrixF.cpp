...
{
  //	UNREFERENCED_PARAMETER(pConMtx);
  /*
    this->neq = pConMtx->N();
...
BOOL SparseMatrixF::IsSamePattern(SparseMatrixF* sm)
{
  BOOL isSame = sm->neq == neq 
    && sm->m_bIsSymmertric == m_bIsSymmertric
    && sm->m_eSparseOrientation == m_eSparseOrientation;

  if (!isSame)
    return FALSE;

  ULONG nnz = Nonzeros();
  for (ULONG j=0; j<nnz; j++)
    if (ci[j] != sm->ci[j])
      return false;
...
void SparseMatrixF::AddNumbers (double alfa,double* a)
{
  ULONG nnz = Nonzeros();
  for (ULONG j=0; j<nnz; j++)
    this->a[j] += alfa*a[j];
...
void SparseMatrixF::ScaleMatrix(double alfa)
{
  ULONG nnz = Nonzeros();
  for (ULONG j=0; j<nnz; j++)
    this->a[j] *= alfa;
