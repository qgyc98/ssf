...
/**
  ...
  Created by JK,
*/
thervolisomat::thervolisomat (void)
{
  alpha=0.0;
}
...
/**
  ...
  Created by JK,
*/
void thervolisomat::read (XFILE *in)
{
...
*/
void thervolisomat::print (FILE *out)
{
...
/**
  ...
  Created by JK,
*/
void thervolisomat::matdilat (matrix &d,strastrestate ssst)
{
  switch (ssst){
  case bar:{
    matdilat_bar (d);
...
*/
void thervolisomat::matdilat_bar (matrix &d)
{
  d[0][0] = alpha;
}
...
/**
  ...
  Created by JK, 11.9.2001
*/
void thervolisomat::matdilat_plbeam (matrix &d)
{
  d[0][0] = alpha;
  d[1][1] = 0.0;
...
*/
void thervolisomat::matdilat_plstress (matrix &d)
{
  fillm(0.0,d);
...
/**
  ...
  Created by JK, 19.7.2001
*/
void thervolisomat::matdilat_plstrain (matrix &d)
{
  fillm(0.0,d);
...
/**
  ...
  Created by JK, 19.7.2001
*/
void thervolisomat::matdilat_axi (matrix &d)
{
  fillm(0.0,d);
...
/**
  ...
  Created by JK, 19.7.2001
*/
void thervolisomat::matdilat_plate (matrix &d)
{
  fillm(0.0,d);
...
/**
  ...
  Created by JK, 19.7.2001
*/
void thervolisomat::matdilat_spacestr (matrix &d)
{
  fillm(0.0,d);
...
/**
   ...
   Created by JK, 12.2.2019
*/
void thervolisomat::strains (long ipp)
{
  double dt,dv;
  strastrestate sss;
  ...
  case bar:{
    ...
    //}
  case planestress:{
    Mm->tempstrains[ipp][0]=alpha*dt+dv/2.0;
    Mm->tempstrains[ipp][1]=alpha*dt+dv/2.0;
  ...
  case planestrain:{
    ...
    //}
  case spacestress:{
    Mm->tempstrains[ipp][0]=alpha*dt+dv/3.0;
    Mm->tempstrains[ipp][1]=alpha*dt+dv/3.0;
    Mm->tempstrains[ipp][2]=alpha*dt+dv/3.0;
...
*/
void thervolisomat::cumulstrains (long ipp)
{
  double dt,dv;
  strastrestate sss;
  ...
  case bar:{
    double debil;
    debil=Mm->tempstrains[ipp][0];

    Mm->tempstrains[ipp][0]+=alpha*dt+dv;

    if (ipp==1){
      fprintf (Out,"\n %15.12le  %15.12le %15.12le  %15.12le",Mp->time,debil,dv,Mm->tempstrains[ipp][0]);
    }
    break;
  }
    /*
...
*/
void thervolisomat::give_reqnmq(long *anmq)
{
  anmq[temperature-1] = 1;
  anmq[initial_temperature-1] = 1;
  anmq[volume_change-1] = 1;
}
