...
shelltrm::shelltrm (void)
{
  long i,j;
  
  if (Perlt==NULL)   Perlt = new planeelemrotlt;
  if (Cct==NULL)     Cct = new cctelem;
  ...
  ncompp = new long [nbp];
  ncompp[0]=Cct->ncomp[0];  // =3
  ncompp[1]=Cct->ncomp[1];  // =2
  ...
  for (i=0;i<nbp;i++){
    for (j=0;j<nbp;j++){
      intordsm[i+nbs][j+nbs]=Cct->intordsm[i][j];
    }
  }
...
/**
   ...
   JK, 16. 2. 2020
*/
void shelltrm::codnum (long *cn,long ri)
{
  long i;
...
/**
   ...
   JK, 16. 2. 2020
*/
void shelltrm::coord_transf_matrix (matrix &tran, vector &gx, vector &gy, vector &gz,double zero)
{
  double c,p,l;
...
/**
   ...
   JK, 16. 2. 2020
*/
void shelltrm::local_coordinates (vector &gx,vector &gy,vector &gz,vector &lx,vector &ly,double zero)
{
...
/**
   ...
   and local nodal coordinate sytem
   ...
   9.5.2002
*/
void shelltrm::node_transf_matrix (ivector &nodes,matrix &tmat)
{
  long i,i6,n,m;
...
/**
   ...
   JK, 16. 2. 2020
*/
void shelltrm::elem_transf_matrix (matrix &tran, vector &gx, vector &gy, vector &gz,double zero)
{
  long i,ri,ci;
...
/**
   ...
   4.5.2002
*/
void shelltrm::res_stiffness_matrix (long eid,matrix &sm)
{
  long *cn,transf;
  ...
  Cct->stiffness_matrix (eid,2,2,lsm,x,y);
...
/**
   ...
   JK, 23.9.2008
*/
void shelltrm::res_ip_strains (long lcid,long eid)
{
  ...
  matrix tmat(ASTCKMAT(ndofe,ndofe)),tran(3,3);
  ...
  //  transformation of displacement vector from nodal coordinate system to the global coordinate system
  ...
  Cct->ip_strains (lcid,eid,2,2,x,y,rp);
...
/**
   ...
   JK, 26.9.2008
*/
void shelltrm::res_ip_stresses (long lcid,long eid)
{
  Perlt->compute_nlstress (lcid,eid,0,0);
  Cct->compute_nlstress (lcid,eid,2,2);
}
...
/**
   ...
   JK, 26.9.2008
*/
void shelltrm::compute_nlstress (long lcid,long eid)
{
  Perlt->compute_nlstress (lcid,eid,0,0);
  Cct->compute_nlstress (lcid,eid,2,2);
}
...
/**
   ...
   JK, 23.9.2008
*/
void shelltrm::compute_nlstressincr (long lcid,long eid)
{
  Perlt->compute_nlstressincr (lcid,eid,0,0);
  Cct->compute_nlstressincr (lcid,eid,2,2);
}
...
/**
   ...
   JK, 26.9.2008
*/
void shelltrm::res_internal_forces (long lcid,long eid,vector &ifor)
{
  long *cn,transf;
  ivector nodes (nne);
  vector aux(ASTCKVEC(ndofe));
  ...
  Cct->internal_forces (lcid,eid,2,2,v,x,y);
...
void shelltrm::nod_strains_ip (long lcid,long eid)
{
...
void shelltrm::strains (long /*lcid*/,long /*eid*/)
{

}

void shelltrm::nod_stresses_ip (long lcid,long eid)
{
  ...
  //  contribution from triangular plate element
  Cct->nod_stresses_ip (lcid,eid,2,2);
}


  
void shelltrm::stresses (long /*lcid*/,long /*eid*/)
{
}
...
*/
void shelltrm::inicipval(long eid, long ri, long ci, matrix &nodval, inictype *ictn)
{
  long i, j, k, ipp;
