...
void bbmmat::initval(long ipp, long im, long ido)
{
  ...
  double a, b, fs, ksi, sr, qq, p_atm = 101.3250; //in kPa
  ...
  if (Mm->ip[ipp].eqother[ido+ncompstr+1] == 0.0)  // actual value of p_c was not set
  {
    ...
    //if((pc_0/i1s) < 0)
    //Mm->ip[ipp].other[ido+ncompstr+4] = Mm->ip[ipp].eqother[ido+ncompstr+4] = v_ini     = v_pc0;//debug??!!
    //else
...
/**
   ...
   25.3.2002
*/
double bbmmat::yieldfunction (vector &sig, vector &q)
{
  double i1s,j2s,f;
...
/**
   ...
   4.1.2002
*/
void bbmmat::deryieldfsigma (vector &sig, vector &q, vector &dfds)
{
  double volume,i1s;
...
/**
   ...
   19.12.2002
*/
void bbmmat::dderyieldfsigma (matrix &ddfds)
{
  fillm(0.0, ddfds);
...
/**
   ...
   4.1.2002
*/
void bbmmat::deryieldfq(vector &sig, vector &q, vector &dfq)
{
  //     df
  // ---------- = dfq[0]
  // d\bar{p}_c
...
/**
  ...
  Created by Tomas Koudelka,  09.2012
*/
void bbmmat::deryieldfdsdq(matrix &dfdsdqt)
{  
...
/**
  ...
  Created by Tomas Koudelka,  09.2012
*/
void bbmmat::updateq(long ipp, long ido, vector &/*eps*/, vector &epsp, vector &/*sig*/, vector &q)
{
  vector epst(ASTCKVEC(6));
  vector depsp(epsp.n);
  double v_kappa1, v_lambda1, p1, depsvp, v_ini, pc_new;
  double v_pc, p_atm = 101.3250; //in kPa
...
/**
  ...
  Created by Tomas Koudelka,  09.2012
*/
void bbmmat::matstiff (matrix &d, long ipp, long ido)
{
  double zero=1.0e-20;
...
void bbmmat::nlstressesincr (long ipp, long im, long ido)
  //
{
  double n,v,depss=0.0,depst=0.0,ds,s,kappa_s,lambda_s,p_atm = 101.3250,p,pc_ref; //in kPa
...
/**
  ...
  Created by Tomas Koudelka,  09.2012
*/
void bbmmat::nlstresses (long ipp, long im, long ido)
  //
{
  int errr;
  ...
  */
  ...
  errr = test_math_errel(Mm->elip[ipp]);
  if (errr){
    pnewdt = 0.5;
    Mm->ip[ipp].other[ido+ncomp+14] = pnewdt;
  }
...
double bbmmat::comp_actual_ym(long ipp, long im, long ido)
{
  ...
  // initial Young modulus from elastic material
  double e_ini = Mm->give_initial_ym(ipp, idem, ido+ncompo);
  ...
  // intial modulus returned to the initval function
  //if ((po == 0.0) || (epsv == 0.0))
  //  return e_ini;
  if (po > pc){
    ...
    double v = v_ini*(1.0+epsv);    
    // actual total volumetric stress
    Mm->givestress(0, ipp, sig);
    give_full_vector(sigt, sig, ssst);
    double p = first_invar(sigt);;
    if (p < 0.0)
...
*/
void bbmmat::elmatstiff(matrix &d, long ipp, long im, long ido)
{
  long idem    = Mm->ip[ipp].gemid();
...
/**
  ...
  Created by Tomas Koudelka,  09.2012
*/
void bbmmat::giveirrstrains (long ipp, long ido, vector &epsp)
{
  long i;
  for (i=0;i<epsp.n;i++)
    epsp[i] = Mm->ip[ipp].eqother[ido+i];
...
/**
  ...
  Created by Tomas Koudelka,  09.2012
*/
double bbmmat::give_consparam (long ipp,long ido)
{ 
  long ncompstr;
  double gamma;
...
*/
double bbmmat::give_saturation_degree(long ipp, long ido)
{
  long ncompstr;
  double s;
...
*/
double bbmmat::give_preconspress(long ipp, long ido)
{
  long ncompstr;
  double pc;
...
/**
  ...
  Created by Tomas Koudelka, 8.10.2013  
*/
double bbmmat::give_virgporosity(long ipp, long ido)
{
  long ncompstr;
  double e_lambda1;
...
/**
  ...
  Created by Tomas Koudelka, 8.10.2013  
*/
double bbmmat::give_iniporosity(long ipp, long ido)
{
  long ncompstr;
  double v_ini, n_ini;
...
*/
double bbmmat::give_porosity(long ipp, long ido)
{
  long ncompstr;
  double n;
...
/**
  ...
  Created by Tomas Koudelka 06.2020
*/
double bbmmat::give_strain_vol_rate(long ipp, long ido)
{
  long ncomp = Mm->ip[ipp].ncompstr;
...
/**
  ...
  Created by Tomas Koudelka,  09.2012
*/
void bbmmat::changeparam (atsel &atm,vector &val)
{
  long i;
