...
/**
  ...
  The total number of eqother components ncompeqother= 2*ncompstr+3+nstatev+12
  nstatev is being defined in the generalmod.h (actual value is 10)
  ...
  Generalized stress vector is given according to D. Masin in genralmod.cpp as follows:
  eps_g = {eps_x, eps_y, eps_z, gamma_xy, gamma_xz, gamma_yz, s, T}^T (the ABACUS ordering of stress components is being used)
...
class hypoplast_unsat_exp_therm
{
 public:
  ...
  long rkf23bs(vector &eps, vector &sig, vector &statev, vector &deps, double &dtsub, long &neval, long &nstep, double &mindt, long ipp);
  ...
  long nparams; //number of parameters
  long nstatev;  //number of state variables
  long ngstrain;  //number of generalised strain tensor components
  long ngstress;  //number of generalised stress tensor components
  double *params;
  int nparstatev[4];
  strretalg sra;
