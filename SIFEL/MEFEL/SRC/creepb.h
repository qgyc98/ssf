...
/**
   ...
   internal variables describing history (nrettimes x nc components for this model)
...
class creepb
{
 public:
  creepb (void);
  ~creepb (void);
  void creepinit (long ipp,long ido,double val,nonmechquant nmq);
  void read (XFILE *in);
  long numberOfCreepb ();
  ...
  void phase2 (long ipp, long ido);
  void get_h (long ipp, long ido);
  void get_temp (long ipp, long ido);
  void matstiff (matrix &d, long ipp, long ido);
  void matstiffchange (double &qq, long ipp, long ido);
  ...
  double e0;
  //  Young's modulus of elasticity in time t
  double et;
  //  Poisson's ratio
  double mi;
