...
void gen_elem(void)
{
  ...
  for (i = 0; i < Ndx; i++)
  {
    for (j = 0; j < Ndy; j++)
    {
      ...
      memset(Len[did], 0, sizeof(*Len[did])*2*Nex[i]*Ney[j]);
      for (ii = 0; ii < Nex[i]; ii++)
      {
        for (jj = 0; jj < Ney[j]; jj++)
        {
          ...
          nid1 = (ii + 1) * (Ney[j] + 1) + jj + 1;
          nid2 = ii * (Ney[j] + 1) + jj;
          nid3 = (ii + 1) * (Ney[j] + 1) + jj;
          El[ie][0] = nid1;
          El[ie][1] = nid2;
          El[ie][2] = nid3;
          Len[did][lie] = ie;
