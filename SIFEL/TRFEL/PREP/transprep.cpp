...
int main (int argc,char *argv[])
{
  ...
  if (argc < 3){
    ...
    delete Gtt;  delete Top;  delete Tt;  delete Tm;  delete Tc;  delete Tb;
    delete Tp;
    return(1);
  }
  in = xfopen(argv[1],"r");
  ...
  if (xfscanf(in,"%+k", "textout") == 0) // docasna uprava pro Maderu, mozno odstranit az budou vyresena klicova slova ve funkci Outdt->print
    in->kwdmode = sect_mode_ignore;
  xf_resetsec(in);
  Outdt->read(in);
  xfclose(in);
  Outdt->conv_sel_prop(Top);
