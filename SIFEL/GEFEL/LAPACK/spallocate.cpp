...
//static char RCSid[] = "@(#)$Header: spAllocate.c,v 1.2 88/06/18 11:12:22 kundert Exp $";
#endif
/*
...
/* Initialize matrix */
    Matrix->ID = SPARSE_ID;
    Matrix->Complex = 0;
    Matrix->PreviousMatrixWasComplex = 0;
    Matrix->Factored = 0;
    Matrix->Elements = 0;
    Matrix->Error = *pError;
    Matrix->Fillins = 0;
    ...
    Matrix->NumberOfInterchangesIsOdd = 0;
    Matrix->Partitioned = 0;
    Matrix->RowsLinked = 0;
...
/* Sequentially step through the list of allocated pointers freeing pointers along the way. */
...
int spError(MatrixPtr Matrix )
{
    if (Matrix != NULL)
    {
		ASSERT(Matrix->ID == SPARSE_ID); return Matrix->Error;
    }
    else return spNO_MEMORY;   /* This error may actually be spPANIC, no way to tell. */
}
/*
 ...
 */
int spGetSize(MatrixPtr Matrix, BOOLEAN External)
{
    ASSERT( IS_SPARSE( Matrix ) );
#if TRANSLATE
    if (External)
