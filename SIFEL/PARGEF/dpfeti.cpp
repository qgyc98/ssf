...
void dpfeti::arrmatrix (double *condmat,long *domproc,FILE *out)
{
  ...
  if (myrank==0){
    ...
    arr->setval (ssle);
    arr->initiate (ptop,tncdof,rsmstor,1,out);
    arr->prepmat (0.0,1);
			    ...
			    double *condmat,double *condvect,double *rhs,long *domproc,FILE *out)
{
  ...
  //  (K_{rr}^{j})^{-1} . f_{r}^{j}
  gm->condense (top,condmat,buff,lhs,zaloha,ncdof,3,out);
  delete [] zaloha;
...
void dpfeti::rhs_dpfeti (gtopology *top,long *domproc,gmatrix *gm,FILE *out)
{
  ...
  */
  ...
  gm->condense (top,buff,buff,wr,ur,ncdof,3,out);
...
void dpfeti::matxvect (gtopology *top,long *domproc,
		       gmatrix *gm,double *input,double *output,FILE *out)
{
  long i,j,gndofe,*cn;
  ...
  gm->condense (top,buff,buff,vr,ur,ncdof,3,out);
...
void dpfeti::cg (gtopology *top,long *domproc,gmatrix *gm,long iv,FILE *out)
{
  ...
  matxvect (top,domproc,gm,cglhs,p,out);
  ...
  for (i=0;i<nicgdpfeti;i++){
    ...
    matxvect (top,domproc,gm,d,p,out);
...
void dpfeti::corner_displ (gtopology *top,long *domproc,gmatrix *gm,FILE *out)
{
  long i,j,gndofe,*cn;
...
void dpfeti::compute_displ (gtopology *top,gmatrix *gm,long *domproc,
			    double *subdispl,double *rhs,FILE *out)
{
  long i,j,gndofe,*cn;
  double *ur,*vr,*buff;
  MPI_Status stat;
  ...
  gm->condense (top,buff,buff,subdispl,rhs,ncdof,3,out);
			   ...
			   double *lhs,double* rhs,FILE *out,long /*mespr*/)
{
  ...
  //  matrix condensation
  gm->condense (top,condmat,condvect,lhs,rhs,ncdof,1,out);
  ...
  corner_displ (top,domproc,gm,out);
  ...
  compute_displ (top,gm,domproc,lhs,rrhs,out);
  ...
  if (myrank==0){
    ...
    long *fact,*cp,*arrasem,*tottime;
    fact=new long [nproc];
    cp=new long [nproc];
    arrasem = new long [nproc];
    ...
    arrasem[j]=buff[2];
    cp[j]=buff[4];
    tottime[j]=buff[0]+buff[1]+buff[2]+buff[3]+buff[4]+buff[5]+buff[6];
    ...
    long minfact,maxfact,mincp,maxcp,minarr,maxarr,mintot,maxtot;
    maxfact=0;
    maxcp=0;
    maxarr=0;
    maxtot=0;
